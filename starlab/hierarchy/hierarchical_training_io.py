"""Build and write hierarchical training run JSON artifacts (M43)."""
...
from starlab.hierarchy.hierarchical_training_models import (
    ...
    HIERARCHICAL_TRAINING_RUN_REPORT_VERSION,
)
...
def write_hierarchical_training_artifacts(
    *,
    ...
    run_path = output_dir / HIERARCHICAL_TRAINING_RUN_FILENAME
    rep_path = output_dir / HIERARCHICAL_TRAINING_RUN_REPORT_FILENAME
...
def minimal_report_from_run(run: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "report_version": HIERARCHICAL_TRAINING_RUN_REPORT_VERSION,
        "training_run_sha256": run["training_run_sha256"],
        "run_id": run["run_id"],
        "training_program_contract_sha256": run["training_program_contract_sha256"],
        "summary": {
            "manager_model_family_id": run["manager_model_family_id"],
            "worker_model_family_id": run["worker_model_family_id"],
        ...
        "delegate_coverage_summary": run["delegate_coverage"],
        "feature_schema_summary": {
            ...
            "coarse_label_count": len(run["feature_schema"]["coarse_label_vocabulary"]),
        },
        ...
        "provenance": {
            ...
            "delegate_policy_id": run["delegate_policy_id"],
            "interface_trace_schema_version": run["interface_trace_schema_version"],
