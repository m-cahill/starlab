"""CLI: emit px1_demo_readiness_protocol.json + report (PX1-M03)."""
...
from starlab.sc2.px1_demo_readiness_models import (
    PX1_DEMO_READINESS_PROTOCOL_FILENAME,
    PX1_DEMO_READINESS_PROTOCOL_REPORT_FILENAME,
)
...
def write_px1_demo_readiness_protocol_artifacts(
    *,
    input_path: Path,
    output_dir: Path,
) -> tuple[Path, Path]:
    ...
    protocol, report = px1_demo_readiness_protocol_bundle(
        input_obj=obj,
        input_sha256=input_sha256,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    p_path = output_dir / PX1_DEMO_READINESS_PROTOCOL_FILENAME
    r_path = output_dir / PX1_DEMO_READINESS_PROTOCOL_REPORT_FILENAME
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Emit deterministic px1_demo_readiness_protocol.json + report (PX1-M03 — bounded "
            "demo-readiness remediation protocol freeze)."
        ),
    )
    p.add_argument(
        "--input",
    ...
    try:
        out_p, out_r = write_px1_demo_readiness_protocol_artifacts(
