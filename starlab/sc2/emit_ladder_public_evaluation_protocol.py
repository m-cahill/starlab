"""CLI: emit ladder_public_evaluation_protocol.json + report (M59)."""
...
def write_ladder_public_evaluation_protocol_artifacts(
    *,
    input_path: Path,
    output_dir: Path,
) -> tuple[Path, Path]:
    ...
    protocol, report = ladder_public_evaluation_protocol_bundle(
        input_obj=obj,
        input_sha256=input_sha256,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    p_path = output_dir / LADDER_PUBLIC_EVALUATION_PROTOCOL_FILENAME
    r_path = output_dir / LADDER_PUBLIC_EVALUATION_PROTOCOL_REPORT_FILENAME
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Emit deterministic ladder_public_evaluation_protocol.json + report (M59 — bounded "
            "charter only)."
        )
    )
    p.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON file with protocol fields (subject, surface kind, venue, non-claims, ...).",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
    ...
    try:
        out_p, out_r = write_ladder_public_evaluation_protocol_artifacts(
