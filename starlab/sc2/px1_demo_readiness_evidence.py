"""Build deterministic px1_demo_readiness_evidence.json + report (PX1-M03)."""
...
from starlab.sc2.px1_demo_readiness_models import (
    ...
    PX1_DEMO_READINESS_EVIDENCE_REPORT_SCHEMA_VERSION,
    PX1_DEMO_READINESS_EVIDENCE_SCHEMA_VERSION,
...
def _evaluate_candidate_vs_frozen(
    *,
    ...
    """Return (satisfies_thresholds, failure_reasons). Same semantics as PX1-M02 play-quality."""
...
def _check_demo_protocol_header(protocol_obj: dict[str, Any]) -> None:
    if protocol_obj.get("contract_id") != PX1_DEMO_READINESS_PROTOCOL_CONTRACT_ID:
        msg = "protocol contract_id mismatch"
        raise ValueError(msg)
    if protocol_obj.get("schema_version") != PX1_DEMO_READINESS_PROTOCOL_SCHEMA_VERSION:
        msg = "protocol schema_version mismatch"
        raise ValueError(msg)


def validate_demo_readiness_evaluation_input(
    *,
    ...
    _check_demo_protocol_header(protocol_obj)
    ...
    if status not in (
        ...
        msg = (
            "selection.status must be demo-ready-candidate-selected|"
            "no-demo-ready-candidate-within-scope"
        )
        raise ValueError(msg)
    rationale = _as_str(sel.get("rationale", ""), "selection.rationale")
    selected_id = sel.get("selected_candidate_id")
    if status == "demo-ready-candidate-selected":
        if not isinstance(selected_id, str) or not selected_id.strip():
            msg = (
                "selection.selected_candidate_id required when status is "
                "demo-ready-candidate-selected"
            )
            raise ValueError(msg)
...
def px1_demo_readiness_evidence_bundle(
    *,
    protocol_obj: dict[str, Any],
    ...
    normalized = validate_demo_readiness_evaluation_input(
    ...
    if sel_status == "demo-ready-candidate-selected":
        if not selected_id:
            selection_consistent = False
            consistency_notes.append(
                "status demo-ready-candidate-selected but no selected_candidate_id",
            )
        else:
            match_blocks = [
            ...
            if len(match_blocks) != 1:
                ...
                if not ok:
                    selection_consistent = False
                    consistency_notes.append(
                        "demo-ready selected but candidate does not satisfy frozen thresholds",
                    )
    elif sel_status == "no-demo-ready-candidate-within-scope" and selected_id:
        selection_consistent = False
        consistency_notes.append(
            "no-demo-ready-candidate-within-scope but selected_candidate_id is set",
        )

    evidence_body: dict[str, Any] = {
        "contract_id": PX1_DEMO_READINESS_EVIDENCE_CONTRACT_ID,
        "evaluation": normalized,
        "generated_attribution": {
            "protocol_canonical_sha256": protocol_sha,
            "protocol_contract_id": PX1_DEMO_READINESS_PROTOCOL_CONTRACT_ID,
            "runtime_doc_rel_path": PX1_DEMO_READINESS_RUNTIME_DOC_REL_PATH,
        },
        "px1_m01_anchor": dict(sorted(protocol_obj["px1_m01_anchor"].items())),
        "schema_version": PX1_DEMO_READINESS_EVIDENCE_SCHEMA_VERSION,
    ...
    report: dict[str, Any] = {
        ...
        "schema_version": PX1_DEMO_READINESS_EVIDENCE_REPORT_SCHEMA_VERSION,
