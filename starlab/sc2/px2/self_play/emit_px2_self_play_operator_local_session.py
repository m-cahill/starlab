"""CLI: bounded operator-local multi-run session (PX2-M03 slice 8)."""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "PX2 slice-8 bounded operator-local session: multiple runs under one campaign root; "
            "writes px2_self_play_operator_local_session.json (local-first; not merge-gate CI)."
        ),
    )
    p.add_argument("--corpus-root", type=Path, required=True)
    p.add_argument("--campaign-id", default=DEFAULT_SLICE8_CAMPAIGN_ID)
    p.add_argument(
        "--base-dir",
        type=Path,
        default=None,
    ...
    p.add_argument(
        ...
        nargs="+",
        default=None,
        help="At least two distinct run ids (default: px2_sess_run_00, px2_sess_run_01, …).",
    )
    ...
    try:
        summary = run_bounded_operator_local_session(
        ...
        return 1
    print(f"operator_local_session_sha256={summary['operator_local_session_sha256']}")
    print(f"campaign_root_manifest_sha256={summary['campaign_root_manifest_sha256']}")
    print(f"ordered_run_ids={summary['ordered_run_ids']}")
    return 0
