...
from starlab.sc2.px2.self_play.current_candidate_reanchor_record import (
    REANCHOR_RECORD_VERSION_SLICE13,
...
SLICE13_REANCHOR_CAMPAIGN_PROFILE_ID: Final[str] = "px2_m03_slice13_second_hop_reanchor_v1"
...
def run_bounded_current_candidate_reanchor_after_second_hop(
    *,
    campaign_root: Path,
    campaign_id: str,
    ...
    """Refresh ``px2_self_play_current_candidate.json`` after ``consumed_ok`` slice-13 second hop.
    ...
    **Not** industrial self-play; **not** **PX2-M04** exploit closure; **not** merge-gate CI.
    ...
    cont_path = root / CONTINUATION_RUN_JSON
    base_non_claims = [
        "Slice-13 post-second-hop current-candidate re-anchor — not industrial execution.",
        "Pointer refresh after second bounded hop; not ladder strength; not PX2-M04.",
        "Not merge-gate default CI proof.",
    ]
    ...
    def _emit_reanchor_only(
        *,
        ...
        return build_px2_self_play_current_candidate_reanchor_artifacts(
            execution_kind=EXECUTION_KIND_SLICE13_REANCHOR,
            campaign_id=campaign_id,
            campaign_profile_id=SLICE13_REANCHOR_CAMPAIGN_PROFILE_ID,
            campaign_root_resolved=root,
            reanchor_rule_id=REANCHOR_RULE_POST_SECOND_HOP_STUB,
            prior_current_candidate_sha256=prior_cc_sha,
            prior_continuation_run_sha256=prior_cont_sha,
            continuation_run_id=continuation_run_id,
            refreshed_current_candidate_sha256=refreshed_sha,
            ...
            rejection_reasons=reasons,
            non_claims=base_non_claims,
            current_candidate_reanchor_record_version=REANCHOR_RECORD_VERSION_SLICE13,
    ...
    consumption = str(cont_rec.get("consumption_status", ""))
    continuation_run_id = str(cont_rec.get("continuation_run_id", ""))
    exec_kind = str(cont_rec.get("execution_kind", ""))
    ...
    if exec_kind != EXECUTION_KIND_SLICE13:
        cc0 = load_px2_self_play_current_candidate(root)
        cc_sha = str(cc0.get("current_candidate_sha256", "")) if cc0 else ""
        rm, rr = _emit_reanchor_only(
            prior_cc_sha=cc_sha,
            ...
            consumption_observed=consumption,
            status="rejected",
            reasons=["continuation_execution_kind_not_slice13_second_hop"],
            contract_sha=contract_sha,
            opp_sha=opp_sha,
            man_sha=man_sha,
        )
        ...
        return {
            ...
            "current_candidate_reanchor_sha256": rm["current_candidate_reanchor_sha256"],
            "refreshed_current_candidate_sha256": None,
            "rejection_reasons": ["continuation_execution_kind_not_slice13_second_hop"],
        }
    ...
    prior_cc_sha = str(prior_cc.get("current_candidate_sha256", ""))
    if (
        str(prior_cc.get("current_candidate_record_version", ""))
        != CURRENT_CANDIDATE_RECORD_VERSION_SLICE12
    ):
        rm, rr = _emit_reanchor_only(
            prior_cc_sha=prior_cc_sha,
            ...
            consumption_observed=consumption,
            status="rejected",
            reasons=["current_candidate_not_post_slice12_record_version"],
            contract_sha=contract_sha,
            opp_sha=opp_sha,
            man_sha=man_sha,
        )
        ...
        return {
            ...
            "current_candidate_reanchor_sha256": rm["current_candidate_reanchor_sha256"],
            "refreshed_current_candidate_sha256": None,
            "rejection_reasons": ["current_candidate_not_post_slice12_record_version"],
        }
    ...
    lineage = {
        "reanchor_interpretation": (
            "Post-second-hop pointer refresh — prior slice-12 candidate, first-hop continuation, "
            "slice-12 re-anchor, and second-hop continuation seals bound here."
        ),
        "prior_post_slice12_current_candidate_sha256": prior_cc_sha,
        "prior_first_hop_continuation_run_sha256": prior_first_hop_continuation_run_sha256,
        "prior_slice12_reanchor_sha256": prior_slice12_reanchor_sha256,
        "second_hop_continuation_run_sha256": prior_cont_sha,
        "continuation_run_id": continuation_run_id,
        "prior_rule_id": str(prior_cc.get("current_candidate_rule_id", "")),
    }

    cc_non_claims = [
        "Slice-13 refreshed current-candidate pointer — not industrial execution.",
        "Not global best policy; not PX2-M04 exploit closure.",
        "Not merge-gate default CI proof.",
    ]
    ...
    cm, cr = build_px2_self_play_current_candidate_artifacts(
        ...
        opponent_pool_identity_sha256=opp_sha,
        campaign_root_manifest_sha256=man_sha,
        current_candidate_rule_id=CURRENT_CANDIDATE_RULE_REANCHOR_FROM_SECOND_HOP_STUB,
        current_run_id_after_transition=continuation_run_id,
        anchor=anchor,
        weight_identity=dict(weight_identity),
        weight_bundle_ref=weight_bundle_ref_val,
        ...
        source_receipt_lineage=lineage,
        non_claims=cc_non_claims,
        record_version=CURRENT_CANDIDATE_RECORD_VERSION_SLICE13,
    )
    write_json(root / CURRENT_CANDIDATE_JSON, cm)
    write_json(root / "px2_self_play_current_candidate_report.json", cr)
