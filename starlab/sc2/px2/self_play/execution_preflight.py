"""Operator-local execution preflight receipts (PX2-M03 slice 3; slice-6 logical seal)."""
...
from starlab.sc2.px2.self_play.path_identity import build_preflight_seal_basis
...
def run_execution_preflight(
    *,
    ...
    ``preflight_sha256`` seals ``preflight_seal_basis`` only (logical paths, no absolute
    machine paths, no environment-specific versions). Advisory absolute paths and full
    check details appear in the emitted JSON and report but are **not** part of the seal.
    ...
    non_claims = [
        ...
        (
            "Preflight seal uses slice-6 logical path identity (no absolute paths in "
            "preflight_seal_basis); not machine-portable in every sense."
        ),
    ]

    seal_basis = build_preflight_seal_basis(
    ...
    seal = _seal_preflight_body(seal_basis)

    wb = weight_bundle_ref.strip() if weight_bundle_ref else None
    preflight: dict[str, Any] = {
        "contract_id": PX2_SELF_PLAY_EXECUTION_PREFLIGHT_CONTRACT_ID,
        "preflight_seal_basis": seal_basis,
        ...
        "weight_bundle_ref": wb,
        "torch_seed": torch_seed,
        "device_intent": device_intent,
        "map_location": map_location,
        "preflight_ok": seal_basis["preflight_ok"],
        "non_claims": non_claims,
