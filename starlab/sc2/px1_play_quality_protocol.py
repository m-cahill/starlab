"""Build deterministic px1_play_quality_protocol.json + report (PX1-M02)."""
...
from starlab.sc2.px1_play_quality_models import (
    ...
    PX1_PLAY_QUALITY_PROTOCOL_REPORT_SCHEMA_VERSION,
...
def validate_protocol_input(obj: dict[str, Any]) -> dict[str, Any]:
    """Validate and normalize protocol input."""

    profile = _as_str(obj.get("protocol_profile_id", ""), "protocol_profile_id")
    if profile != PX1_M02_PROTOCOL_PROFILE_BOUNDED_LOCAL_LIVE_V1:
        msg = (
            f"unsupported protocol_profile_id {profile!r}; only "
            f"{PX1_M02_PROTOCOL_PROFILE_BOUNDED_LOCAL_LIVE_V1!r} is supported in PX1-M02 v1"
        )
        raise ValueError(msg)

    cid = obj.get("contract_id")
    if cid is not None:
        if cid != PX1_PLAY_QUALITY_PROTOCOL_CONTRACT_ID:
            msg = f"contract_id must be {PX1_PLAY_QUALITY_PROTOCOL_CONTRACT_ID!r} when set"
            raise ValueError(msg)

    pv = _as_str(obj.get("protocol_version", ""), "protocol_version")
    if pv != PX1_PLAY_QUALITY_PROTOCOL_SCHEMA_VERSION:
        msg = f"protocol_version must be {PX1_PLAY_QUALITY_PROTOCOL_SCHEMA_VERSION!r}"
        raise ValueError(msg)
    ...
    if runtime_doc != PX1_PLAY_QUALITY_RUNTIME_DOC_REL_PATH:
        msg = (
            f"runtime_doc_rel_path must be {PX1_PLAY_QUALITY_RUNTIME_DOC_REL_PATH!r} for PX1-M02 v1"
        )
        raise ValueError(msg)
    ...
    for key in required_fp_keys:
        ...
        if key == "selection_rule":
            ...
            s = _as_str(val, f"frozen_parameters.{key}")
            if s != "local_live_sc2":
                msg = f"frozen_parameters.{key} must be 'local_live_sc2' for PX1-M02 v1"
                raise ValueError(msg)
            frozen_parameters[key] = s
        elif key in (
            "minimum_candidates_evaluated",
    ...
    protocol: dict[str, Any] = {
        ...
        "contract_id": PX1_PLAY_QUALITY_PROTOCOL_CONTRACT_ID,
        ...
        "schema_version": PX1_PLAY_QUALITY_PROTOCOL_SCHEMA_VERSION,
    }
    return dict(sorted(protocol.items()))


def px1_play_quality_protocol_bundle(
    *,
    ...
    protocol = validate_protocol_input(input_obj)
    canonical_sha = sha256_hex_of_canonical_json(protocol)
    report: dict[str, Any] = {
        "input_sha256": input_sha256,
        "protocol_canonical_sha256": canonical_sha,
        "schema_version": PX1_PLAY_QUALITY_PROTOCOL_REPORT_SCHEMA_VERSION,
    }
