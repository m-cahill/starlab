"""CLI: emit px1_play_quality_evidence.json + report (PX1-M02)."""
...
from starlab.sc2.px1_play_quality_evidence import px1_play_quality_evidence_bundle
from starlab.sc2.px1_play_quality_models import (
    PX1_PLAY_QUALITY_EVIDENCE_FILENAME,
    PX1_PLAY_QUALITY_EVIDENCE_REPORT_FILENAME,
)
...
def write_px1_play_quality_evidence_artifacts(
    *,
    protocol_path: Path,
    ...
    evidence, report = px1_play_quality_evidence_bundle(
        protocol_obj=protocol_obj,
        evaluation_input_obj=eval_obj,
        evaluation_input_sha256=eval_sha,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    e_path = output_dir / PX1_PLAY_QUALITY_EVIDENCE_FILENAME
    r_path = output_dir / PX1_PLAY_QUALITY_EVIDENCE_REPORT_FILENAME
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Emit deterministic px1_play_quality_evidence.json + report from a protocol artifact "
            "and an evaluation-results JSON file (PX1-M02)."
        ),
    )
    p.add_argument(
        "--protocol",
        type=Path,
        required=True,
        help="Path to px1_play_quality_protocol.json (emitted by protocol emitter).",
    )
    p.add_argument(
        "--evaluation-input",
        type=Path,
    ...
    try:
        e, r = write_px1_play_quality_evidence_artifacts(
