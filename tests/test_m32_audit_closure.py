...
def test_smoke_collection_count_in_target_band() -> None:
    """Bounded fast lane: ~25–130 smoke tests (see M32 plan; M33+ closeout adds governance rows).
    ...
    **M48/M49/M58 V15**) smoke).
    ...
    assert m, out
    n = int(m.group(1))
    _msg = f"smoke count {n} outside 25–145 band (M32; V15-M58 governance bump)"
    assert 25 <= n <= 145, _msg
