...
def test_smoke_collection_count_in_target_band() -> None:
    """Bounded fast lane: ~25–113 smoke tests (see M32 plan; M33+ closeout adds governance rows).

    Upper band widened over time as M10–M16, M17–M25, M27–M31, **M32** smoke governance rows landed
    (+ M20/M21/M23/M24/M25/M31/M32/M33/M34/M38 smoke).
    """
    proc = subprocess.run(
    ...
    assert m, out
    n = int(m.group(1))
    _msg = f"smoke count {n} outside 25–113 band (M14–M16 governance rows; band widened by count)"
    assert 25 <= n <= 113, _msg
