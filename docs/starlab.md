...
| **v1.5 (V15)** | **`V15-M00` closed** (charter + long GPU run **gate** + public **`docs/starlab-v1.5.md`**) — **not** long run completed in M00. **`V15-M01` next** (not started). | Full GPU training session + **strong-agent** benchmark + replay-native **XAI** + checkpoint/data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not in **PX2** | **V15-M01**+ |
