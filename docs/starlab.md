...
| Last closed milestone | **M41** — Replay-Imitation Training Pipeline v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M42** — Learned-Agent Comparison Harness v1 (**in progress** on branch; Phase VI; see §11). |
| Next planned training milestone | **M43** — Hierarchical Training Pipeline v1 (**planned**; **not** claimed until closed on `main` with authoritative CI). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M43+** hierarchical / live-play / self-play **product** results (until each milestone closes on `main`), **operating manual v1** — see §11 *explicit non-claims*. |
...
- learned-agent comparison harness (**M42** — **in progress**: compare **M27** vs **M41** on **M28** offline metrics; **local-first**; CI **fixture-only**; **no** live-play expansion here)
...
**Phase VI artifact family (compact):** **M40** — training-program **contract** artifacts (`agent_training_program_contract.json` / report); **M41** — **training run** artifacts (`replay_imitation_training_run.json` / report + optional local weights); **M42** — **comparison** artifacts (`learned_agent_comparison.json` / report). Training and comparison remain **local-first**; CI stays **fixture-only** (no GPU training, no live SC2, no weights in-repo).
...
| M42 | Learned-Agent Comparison Harness v1 | VI | In progress | v0.0.42-m42 | — |
...
| M42 | Compare frozen M27 baseline and M41 training-run candidates on the governed offline evaluation surface (**not** benchmark integrity) |
...
| **M42** | **Learned-agent comparison harness v1** (planned; **not** claimed until closed on `main`). |
...
### M42 — Learned-Agent Comparison Harness v1 — **in progress** (current)

**Objective:** Deliver the first **deterministic, governed comparison layer**: compare **M27** frozen baseline and **M41** replay-imitation training-run candidates on the shared **M28** offline metric surface (accuracy, macro-F1, fallback rate, example count), emit `learned_agent_comparison.json` / `learned_agent_comparison_report.json` with pairwise deltas, **ranking_policy_id** `starlab.m42.ranking.accuracy_macro_f1_candidate_id_v1`, and explicit non-claims. **TrainedRunPredictor** loads M41 local `joblib` sidecars; **not** pre-computed external metrics as v1 input.

**Candidate sources:** `m27_frozen_baseline` (`replay_imitation_baseline.json`); `m41_training_run` (`replay_imitation_training_run.json` + run directory with weights).

**Ranking:** accuracy ↓, macro_f1 ↓, candidate_id ↑; fallback_rate recorded but not used in ranking (v1).
...
**Explicit non-claims:** **not** benchmark integrity; **not** replay↔execution equivalence; **not** live SC2 in CI; **not** M43 hierarchical training; **not** statistical significance or leaderboard claims. Plan: `docs/company_secrets/milestones/M42/M42_plan.md`. Runtime: `docs/runtime/learned_agent_comparison_harness_v1.md`. **Not** closed on `main` until merge + authoritative CI.
