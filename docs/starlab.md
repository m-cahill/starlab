...
| Last closed milestone (PX2 arc) | **PX2-M02** — Neural Bootstrap from Replays — **closed** on `main` ([PR #96](https://github.com/m-cahill/starlab/pull/96)). **Prior:** **PX2-M01** — Full Terran Runtime & Action Surface — **closed** on `main` ([PR #95](https://github.com/m-cahill/starlab/pull/95)). **Prior:** **PX2-M00** — Autonomous Full-Game Agent Charter & Success Criteria — **closed** on `main` (governance closeout [PR #94](https://github.com/m-cahill/starlab/pull/94)). **PX2-M00** — governance charter only — **not** Terran runtime. **PX2-M01** — Terran runtime/action substrate — **not** training. **PX2-M02** — first replay-bootstrap learning over **M01** — **not** self-play; **not** strength proof. |
| Current milestone | **`PX2-M03`** — Industrial Self-Play Campaign — **open** on `main` **after** positive readiness in **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**. **Implementation slices on `main`:** (1) contract/profile, bridge, opponent-pool stub, fixture smoke; (2) **execution skeleton**, run manifest, **checkpoint/eval receipts**; (3) **operator-local execution preflight** + bounded **real-weights** operator-local smoke; (4) **multi-step continuity** + sealed receipt linkage + **promotion/rollback** receipt surfaces; (5) **operator-local campaign-root manifest** + **expanded opponent pool** + deterministic **opponent rotation/selection recording**; (6) **preflight seal normalization** (`preflight_seal_basis`) + **canonical operator-local campaign-root smoke path**; (7) **bounded operator-local real-run** execution record (`px2_self_play_operator_local_real_run.json`); (8) **bounded operator-local multi-run session** (`px2_self_play_operator_local_session.json`, multiple `runs/<run_id>/`); (9) **bounded session transition** (`px2_self_play_operator_local_session_transition.json`); (10) **current-candidate carry-forward** (`px2_self_play_current_candidate.json`); (11) **continuation run consuming current-candidate** (`px2_self_play_continuation_run.json`); (12) **post-continuation current-candidate re-anchoring** (`px2_self_play_current_candidate_reanchor.json`) — runtime **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** — **not** industrial Blackwell-scale execution (**still** within **`PX2-M03`**). **PX2-M03** does **not** auto-open from **PX2-M02** closeout alone. **Last closed PX2:** **`PX2-M02`** ([PR #96](https://github.com/m-cahill/starlab/pull/96)) — replay-bootstrap only; **not** self-play evidence. **`PX2-M04`** / **`PX2-M05`** — **planned only**. **PX1-M04** — **closed** ([PR #93](https://github.com/m-cahill/starlab/pull/93)) — **does not** auto-open **`PX1-M05`** or **v2**. **`PX1-M05`** remains **optional / not yet opened**. **v2** is **not** opened. |
...
| Next planned follow-on | **`PX2-M04`** — Promotion, Evaluation, and Exploit Closure — next **recorded** follow-on after **`PX2-M03`** on the **PX2** roadmap — **planned only** — **not** opened automatically by **`PX2-M03`**. **`PX1-M05`** remains **optional / not yet opened**. **`PX1-M04`** **does not** open **PX1-M05** or **v2**. **`docs/starlab-vision.md`** **v2** directions remain **outside** the closed **M00–M61** arc until **explicit** recharter — **v2** is **deferred** / **not** opened. |
...
| **PX2** | **PX2-M00**–**PX2-M02** **closed**; **`PX2-M03`** **open** (industrial self-play campaign — operator-local / Blackwell-class intent — **not** strength proof yet) | Promotion/exploit closure (**PX2-M04**); demo refresh (**PX2-M05**) — **later** | **PX2-M00**–**PX2-M05** (roadmap) |
...
**Ledger truths (PX1 phase — historical; program `current milestone` = quick scan / §11 / Post-PX1 (PX2)):**
...
| **Later `PX2-M03` execution** | Operator-local industrial self-play (Blackwell-class **intent**), long-run checkpoints/evals/promotion as implemented — **not** claimed by slices 1–12 |

### PX2 roadmap (**PX2-M00**–**PX2-M02** **closed** on `main`; **`PX2-M03`** **open** on `main`; **`PX2-M04`**–**`PX2-M05`** **planned**)
...
| `PX2-M03` | Industrial Self-Play Campaign | **open** on `main` | Preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**; **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** — slices 1–12 (through post-continuation current-candidate re-anchoring). Industrial run — **later** in milestone — **not** **PX2-M04**; **not** demo refresh |
...
PX2 **does not** silently authorize: ladder/public strength proof; benchmark universality; multi-race coverage; **v2** opening; automatic Blackwell execution; or “charter merged” ⇒ “strong agent exists.” **PX2-M00** does **not** implement the Terran action surface, replay training, or industrial campaigns. **`PX2-M01`** **delivered** the **Terran runtime/action substrate** — **not** replay-bootstrap training (**PX2-M02**), **not** industrial Blackwell campaigns (**PX2-M03**), **not** autonomous strength proof.

**Ledger truths (PX2 phase, `current milestone` = **`PX2-M03`**):**

- **PX2** is **separate** from **PX1** — **PX1-M04** did **not** open **PX2**; **PX2** was **opened** by explicit governance (**PX2-M00** on [PR #94](https://github.com/m-cahill/starlab/pull/94)).
...
- **`PX2-M03`** **open** on `main` — industrial self-play **campaign** milestone (operator-local / Blackwell-class intent) — **readiness** **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`** — **slice 1** + **slice 2** + **slice 3** + **slice 4** + **slice 5** + **slice 6** + **slice 7** + **slice 8** + **slice 9** + **slice 10** + **slice 11** + **slice 12** (preflight + bounded operator-local smoke + multi-step continuity + promotion/rollback receipts + campaign-root manifest + opponent rotation + preflight seal normalization + canonical operator-local smoke path + bounded real-run execution record + bounded multi-run session + bounded session transition + current-candidate carry-forward + continuation run consuming current-candidate + post-continuation current-candidate re-anchoring) on `main` — **not** industrial Blackwell-scale completion; **not** merge-gate long-run execution; **not** **PX2-M04** promotion/exploit closure.
...
| PX2-M03 | Industrial self-play **campaign** (operator-local / Blackwell-class intent; checkpoints/evals/promotion/rollback/opponent pool; slices 1–12 through post-continuation re-anchoring — **open** on `main` — preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**) — **not** M04 promotion closure; **not** campaign **evidence** by opening alone |
...
**Current:** **`PX2-M03`** — Industrial Self-Play Campaign — **open** on `main` (readiness / preflight: **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**). **PX2-M03 requires a positive readiness/preflight decision after PX2-M02; it does not auto-open merely because PX2-M02 closed.** **Last closed PX2:** **`PX2-M02`** ([PR #96](https://github.com/m-cahill/starlab/pull/96)). **`PX2-M04`** / **`PX2-M05`** — **planned only**. **`PX1-M05`** **optional / not yet opened**; **v2** **not** opened.

### PX2-M03 — Industrial Self-Play Campaign — **open** on `main`

**Intent:** First **industrial self-play** milestone: **operator-local**, **Blackwell-class** compute **intended**; **checkpoint** cadence, **eval** cadence, **promotion / rollback** rules, **opponent pool** / **anti-collapse** posture — per charter + readiness sketch — **not** **PX2-M04** (promotion / exploit closure); **not** **PX2-M05** (demo refresh).
...
**Non-claims:** **not** self-play **evidence** in-repo; **not** Blackwell run **completed** by opening; **not** strength proof; slices 1–12 **not** substitute for industrial **`PX2-M03`** execution phase.
...
**Post-closeout ledger (historical):** After **`PX2-M02`** closeout, **`current milestone`** was **`None`** until **`PX2-M03`** opened by **readiness** + governance (**`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**). **`PX1-M05`** / **v2** **not** opened by **`PX2-M02`**.
