...
| Current milestone | **M45** — Self-Play / RL Bootstrap v1 (**stub**; Phase VI; see §11) — **not** claimed **closed** on `main` until merge with authoritative CI. |
| Next planned training milestone | **—** — **M45** is the **only** remaining Phase VI training-track milestone in **M40**–**M45** (**stub**; **not** claimed until closed on `main` with authoritative CI). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M45+** self-play / RL **product** results (until each milestone closes on `main`), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
| **M45** | **Self-play / RL bootstrap v1** — **stub** (**current**; **not** claimed **closed** until merge on `main`; see §11). |
...
### M45 — Self-Play / RL Bootstrap v1 — **stub** (current)

**Objective:** Next Phase VI training-track milestone — **not** implemented in this repository until chartered and merged with authoritative CI.

**Explicit non-claims:** **not** benchmark integrity; **not** replay↔execution equivalence; **not** live SC2 in CI; **not** ladder / public performance — until **M45** closes on `main` with recorded CI. Plan `docs/company_secrets/milestones/M45/M45_plan.md` (**stub**).
