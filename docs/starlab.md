...
**M17 note:** M17 **implementation is complete in-repo** (observation surface contract — schema + report + CLI + fixtures/tests). **Record authoritative PR-head + merge-boundary `main` CI at merge** (see §18 / changelog). “Complete” in the milestone table reflects **governed observation surface contract v1** — deterministic `observation_surface_schema.json` + `observation_surface_schema_report.json`, runtime contract `docs/runtime/observation_surface_contract_v1.md`, product modules under `starlab/observation/`; **semantic upstream is M16 `canonical_state.json`** (not replay bundles directly); **no** canonical-state→observation **materialization** pipeline, **no** perceptual bridge (**M18**), **no** replay parsing / bundle loading in M17 modules — see §6 Phase III artifact row. **Not** mask legality computation, full SC2 action coverage, benchmark integrity, replay↔execution equivalence, exact banked resources, certified fog-of-war truth, or live SC2 in CI.
...
| Observation surface contract v1 (single player-relative frame) | M17 | **In-repo** — narrow JSON Schema + report + fixture validation; upstream M16 canonical state; **not** materialization pipeline, perceptual bridge, or replay↔execution equivalence |
...
### Phase III schema / pipeline / observation glossary (M15–M17)
...
| **M18** | **Perceptual bridge / prototype implementation** (planned — not proved by M17). |
...
**Status:** **Planned** — **current** milestone; **M17** observation surface contract is **implemented in-repo** (record merge/CI in §18 / changelog at merge boundary). **Phase III** is **in progress** (M15–M17 complete in milestone table; M18–M19 planned).

**Goal (high level):** Prototype the **perceptual bridge** — **not** proved in M15 (schema-only), M16 (bundle→frame pipeline), or M17 (observation **contract** only); see Phase III artifact table (§6) and Phase III glossary (§10).

**Primary references:** `docs/company_secrets/milestones/M18/M18_plan.md` (stub); `docs/starlab.md` §10 (proved vs not yet proved).

**Note:** **M17** proves **governed observation surface schema + report + validation** — **not** canonical-state→observation materialization, **not** pixels / image-space transforms, **not** an executable action API.

#### Current milestone — explicit non-claims (standing)

Until **M18** explicitly closes a claim, treat the following as **not proved** for **M18** planning:
...
| M17       | _(pending merge)_ | _(record PR)_ | _(record SHA)_ | Observation surface contract (`observation_surface_schema.json` / `observation_surface_schema_report.json`); **record authoritative PR-head + merge-boundary `main` CI at merge**; see §23 M17 changelog entry |
...
**M17 note:** Evidence column reflects **observation surface schema + report emission**, **jsonschema** validation over fixtures, and full governance CI **on the M17 PR head / merge boundary** when merged (**record at merge**); **not** canonical-state→observation materialization, perceptual bridge (M18), mask legality computation, replay↔execution equivalence, benchmark integrity, exact banked resources, certified fog-of-war truth, or live SC2 in CI.
...
### 2026-04-08 — M17 observation surface contract (implementation + ledger prep)

- Added `docs/runtime/observation_surface_contract_v1.md` and `starlab/observation/` (`observation_surface_schema.json` / `observation_surface_schema_report.json` emission; CLI `python -m starlab.observation.emit_observation_surface_schema`; fixtures under `tests/fixtures/m17/`).
- §6 Phase III compact table: **M17** row; §7 milestone table: **M17** → **Complete**; §10 / §11 / Phase III glossary: **M17** contract proof vs **M18** bridge; **current milestone** → **M18** (stub); score trend + **M17** note; **M18** milestone stubs seeded (`docs/company_secrets/milestones/M18/`).
- **Merge authority:** record **PR / merge commit / authoritative PR-head CI / merge-boundary `main` CI** when M17 merges to `main` (this entry is **not** merge-boundary evidence by itself).
