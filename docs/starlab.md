...
| Current milestone | **`V15-M09`** — *Checkpoint Evaluation and Promotion* — **governance surface implemented (implementation PR pending `main` merge)**; `emit_v15_checkpoint_evaluation`, `emit_v15_checkpoint_promotion_decision`, **`docs/runtime/v15_checkpoint_evaluation_promotion_v1.md`**; **no** strong-agent, human, or XAI claims by default. **Prior (v1.5):** **`V15-M08`** — **closed** on `main` ([PR #133](https://github.com/m-cahill/starlab/pull/133)); **`implementation_ready_waiting_for_operator_run`**; **`long_gpu_run_authorized` false** in fixture. **M08 artifacts:** `emit_v15_long_gpu_training_manifest`, `run_v15_long_gpu_campaign`, **`docs/runtime/v15_long_gpu_campaign_execution_v1.md`**. **Prior closed:** **`V15-M07`** — **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**; **M00–M06** — **`docs/starlab-v1.5.md`**. **PX2** (through **M03**) **closed** / **transition-complete**. **`PX2-M04`/`PX2-M05`** — **planned**; **v2** not opened. |
...
| Next planned follow-on | **`V15-M10`** — *Replay-Native XAI Demonstration* — **not started** (awaits M09+ planning). **V15-M08** long-run **execution** (operator-authorized) remains **separate** from M09’s evaluation surface — see **`docs/starlab-v1.5.md`**. **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only**. **v2** **deferred**. |
...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M08`** **closed** on `main` (M08: [PR #133](https://github.com/m-cahill/starlab/pull/133); M07: [PR #129](https://github.com/m-cahill/starlab/pull/129); M06: [PR #127](https://github.com/m-cahill/starlab/pull/127)); **`V15-M08`** — **`implementation_ready_waiting_for_operator_run`**. **`V15-M09`** — checkpoint evaluation / promotion **surface in flight** (see **`docs/runtime/v15_checkpoint_evaluation_promotion_v1.md`**; merge pending). M07: training-run **receipt** + shakedown **tooling** — **not** long campaign. M06: protocol + `fixture_ci` + optional **operator_declared** — **not** human-panel **execution**; see **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**. | Full GPU run + **strong-agent** eval + XAI + **human-panel** eval — **chartered in v1.5**, not satisfied by **PX2** alone | **M10+** (XAI / human) per roadmap after honest M09 posture |
...
**Current (v1.5):** **`V15-M09`** — *Checkpoint Evaluation and Promotion* — **governance surface in flight (implementation PR pending merge to `main`)**; `emit_v15_checkpoint_evaluation`, `emit_v15_checkpoint_promotion_decision`; **runtime** **`docs/runtime/v15_checkpoint_evaluation_promotion_v1.md`**; default **honest** posture without an M08 campaign receipt: **`blocked_missing_m08_campaign_receipt`** — **not** a strong-agent or human or XAI claim. **Prior:** **`V15-M08`** — *Long GPU Campaign Execution* — **implementation closed** on `main` ([PR #133](https://github.com/m-cahill/starlab/pull/133); merge `33b27738…`; **authoritative PR-head CI** [`24940027890`](https://github.com/m-cahill/starlab/actions/runs/24940027890); **merge-boundary `main` CI** [`24940102671`](https://github.com/m-cahill/starlab/actions/runs/24940102671)); **public closeout** **`implementation_ready_waiting_for_operator_run`**; **no** operator-local long campaign **completed**; **`long_gpu_run_authorized` false** on fixture/public path. **Closed v1.5 line on `main`:** **`V15-M00`**…**`V15-M08`**. **Authority:** **`docs/starlab-v1.5.md`**. M08 runtime: **`docs/runtime/v15_long_gpu_campaign_execution_v1.md`**. M07 (closed) runtime: **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**, [PR #129](https://github.com/m-cahill/starlab/pull/129) — **authoritative PR-head CI** [`24925848388`](https://github.com/m-cahill/starlab/actions/runs/24925848388); **merge-boundary `main` CI** [`24925929052`](https://github.com/m-cahill/starlab/actions/runs/24925929052) — **success**; **operator-local shakedown** status: **tooling-only** / **`operator_local_short_gpu_not_run`**. **`V15-M06`**: **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**; **`V15-M05`**: **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; **`V15-M04`**: **`docs/runtime/v15_xai_evidence_contract_v1.md`**; **`V15-M03`**: **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; **`V15-M02`**: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**; **`V15-M01`**: **`docs/runtime/v15_training_scale_provenance_asset_registers_v1.md`**.
...
### V15-M09 — *Checkpoint Evaluation and Promotion* — **governance surface implemented (PR pending `main`)**

**V15-M09** defines `starlab.v15.checkpoint_evaluation.v1` and `starlab.v15.checkpoint_promotion_decision.v1` with `python -m starlab.v15.emit_v15_checkpoint_evaluation` and `python -m starlab.v15.emit_v15_checkpoint_promotion_decision`. **Not** a completed campaign receipt by itself; default posture remains **`blocked_missing_m08_campaign_receipt`** when M08 has not completed a long run. **V15-M10** — **not started**. See **`docs/starlab-v1.5.md`** (M09 non-claims) and **`docs/runtime/v15_checkpoint_evaluation_promotion_v1.md`**.
