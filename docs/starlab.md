...
| Current milestone | **`V15-M03`** — *Checkpoint Lineage and Resume Discipline* — **not** started (no implementation on `main`; await **explicit** plan approval). **Last closed (v1.5):** **`V15-M02`** on `main` ([PR #118](https://github.com/m-cahill/starlab/pull/118); merge `3f7e226f…`; PR-head CI [`24918006750`](https://github.com/m-cahill/starlab/actions/runs/24918006750); merge-boundary [`24918563270`](https://github.com/m-cahill/starlab/actions/runs/24918563270)). See **`docs/starlab-v1.5.md`**, runtime **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** perform a **long wall-clock GPU training session** as a program deliverable. Default **CI** remains **CPU** fixture self-play / bootstrap paths. **`PX2-M04`** / **`PX2-M05`** — **planned**; **`PX1-M05`** optional; **v2** not opened. |
...
| **v1.5 (V15)** | **`V15-M00`**, **`V15-M01`**, **`V15-M02`** **closed** on `main` (PR #116–#118) — registers + `starlab.v15.training_asset_registers.v1` + `starlab.v15.long_gpu_environment_lock.v1` (**fixture** default); **not** long GPU **training**; **not** claim-critical asset approval. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI** + checkpoint **lineage runtime** + data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not in **PX2** | **V15-M03**+ |
...
**Ledger truths (PX2 phase, **closeout**; program **current** v1.5 = **`V15-M03`** (checkpoint lineage — **not** started); **last closed** v1.5 = **`V15-M02`** / **`V15-M01`** / **`V15-M00`** — authority **`docs/starlab-v1.5.md`**):**
...
**Current (v1.5):** **`V15-M03`** — *Checkpoint Lineage and Resume Discipline* — **not** started (await **explicit** plan approval). **Authority:** **`docs/starlab-v1.5.md`**. **Prior M02** runtime: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**.
