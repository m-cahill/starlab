...
| Last closed milestone | **M44** — Local Live-Play Validation Harness v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M45** — Self-Play / RL Bootstrap v1 (**In progress**; Phase VI; see §11) — **not** claimed **closed** on `main` until merge with authoritative CI. |
| Next planned training milestone | **—** — **M45** is the **final** Phase VI training-track milestone in **M40**–**M45** (**In progress**; **not** claimed **closed** until merge on `main` with authoritative CI). After M45 closeout, the next follow-on is the **post-M45 Phase VI integrated test campaign** (operator-local; **not** default CI) — see `docs/diligence/phase_vi_integrated_test_campaign.md`. |
| Phase VI pipeline (compact) | **M40** contract → **M41** flat training run → **M42** comparison → **M43** hierarchical training → **M44** local live-play → **M45** self-play / RL |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **broad** self-play / RL **product** claims beyond **M45**’s bounded bootstrap (until explicitly closed), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
| M45 | Self-Play / RL Bootstrap v1 | VI | In progress | v0.0.45-m45 | — |
...
| **M45** | **Self-play / RL bootstrap v1** — `starlab.training` bootstrap emitter + M44 harness; `self_play_rl_bootstrap_run.json` / report; optional weighted re-fit (**In progress**; **not** claimed **closed** until merge on `main`; see §11). |
...
### M45 — Self-Play / RL Bootstrap v1 — **In progress** (current)

**Objective:** First **governed** self-play / RL **bootstrap** surface: consume a **governed M43** hierarchical candidate + **local** `joblib` weights; drive **bounded** rollout collection through the **M44** local live-play validation harness; record rollout / reward / provenance metadata; optionally perform **one** conservative **weighted re-fit** over the existing sklearn logistic-regression family; emit deterministic **`self_play_rl_bootstrap_run.json`** / **`self_play_rl_bootstrap_run_report.json`** (+ `bootstrap_dataset.json`, per-episode M44 outputs under `out/rl_bootstrap_runs/<run_id>/episodes/`). CLI: `python -m starlab.training.emit_self_play_rl_bootstrap_run`. Runtime contract: `docs/runtime/self_play_rl_bootstrap_v1.md`. Plan: `docs/company_secrets/milestones/M45/M45_plan.md`.
...
**M40**–**M44** are **closed** on `main`. Until **M45** **merges and closes** on `main` with recorded CI, treat the following as **not proved** by default:

- **M45** Phase VI **training-track product** milestone (**not** claimed until it closes with recorded CI); **M42** comparison artifacts **do not** prove benchmark integrity, statistical significance, or live-play outcomes.
...
### 2026-04-12 — M45 chartered (**In progress** on branch `m45-self-play-rl-bootstrap-v1`)

- **M45** is no longer a **stub** in the ledger: first governed **bootstrap** surface in `starlab.training` (`emit_self_play_rl_bootstrap_run`), runtime contract `docs/runtime/self_play_rl_bootstrap_v1.md`, fixture-only CI validation, optional local weighted re-fit; **post-M45** integrated test campaign note in `docs/diligence/phase_vi_integrated_test_campaign.md`.
- **Not** claimed **closed** on `main` until merge with authoritative CI.
