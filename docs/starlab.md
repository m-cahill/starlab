...
| Current milestone | **M42** — Learned-Agent Comparison Harness v1 (**planned** / stub; Phase VI; see §11). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M42+** comparison / hierarchical / live-play / self-play **product** results (until each milestone closes on `main`), **operating manual v1** — see §11 *explicit non-claims*. |
...
- replay-imitation training pipeline (**M41** — **closed** on `main`: deterministic run/report JSON + local sklearn weights; CI fixture-only; **M42**+ next)
...
### M42 — Learned-Agent Comparison Harness v1 — **planned** (current)

**Objective:** Next Phase VI milestone — deterministic learned-agent comparison harness per plan — **not** implemented until closed on `main` with authoritative CI. Plan: `docs/company_secrets/milestones/M42/M42_plan.md`.
