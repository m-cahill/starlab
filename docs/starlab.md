...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M54** **closed** on `main`; **M55** **stub/planned**; **M56**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M54** — Replay↔execution equivalence audit & acceptance gates v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M55** — Benchmark integrity charter & split-governance controls v1 — **stub/planned** (Phase VII; see §11). **M55** is the **benchmark-integrity** charter milestone — **deferred** until authorized; **not** replay↔execution equivalence (closed under **M52**–**M54**). |
...
| Phase VII pipeline (compact) | **M52**–**M61** — trust, equivalence, benchmark integrity, live SC2-in-CI posture, ladder/public evaluation protocol, audit hardening, v1 release lock — see §6; **M52** ledger + equivalence **charter** **closed** on `main`; **M53** equivalence **evidence** surface **closed** on `main` (bounded profiles); **M54** **closed** on `main` (profile-scoped audit + acceptance gates over M53 evidence); **M55**–**M61** **stub/planned** until chartered/closed. |
...
- **M55** — Benchmark integrity charter & split-governance controls v1 (**stub/planned** — next chartered benchmark-integrity milestone).
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M54** **closed** on `main`; **M55**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M55 | Benchmark integrity charter & split-governance controls v1 — **stub/planned** (benchmark integrity **deferred** to chartered **M55** branch) |
| M56 | Benchmark integrity evidence & reproducibility gates v1 — **stub/planned** |
| M57 | Narrow live SC2 in CI charter & controlled runner v1 — **stub/planned** |
| M58 | Live SC2 in CI hardening & cost guardrails v1 — **stub/planned** |
| M59 | Ladder/public evaluation protocol & evidence surface v1 — **stub/planned** |
...
**Status:** current milestone pointer (**Phase VII**). **M55** is the **benchmark-integrity** charter milestone — **not** started as product work until an authorized **M55** branch; **not** a substitute for **M52**–**M54** replay↔execution equivalence track (charter → evidence → profile-scoped audit). Folder: `docs/company_secrets/milestones/M55/` (stub).
...
- **Post-closeout `main` CI** (ledger + tag push after this closeout commit): record separately when observed — **not** merge-boundary authority vs [`24442394865`](https://github.com/m-cahill/starlab/actions/runs/24442394865) on `773dd19…`. **Non-merge-boundary:** tag push **`v0.0.54-m54`** does not trigger the `CI` workflow (`on.push` is **`branches: [main]`** only) — **unless** pushed with `main` (annotated tag on merge commit).
