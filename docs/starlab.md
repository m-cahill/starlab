...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M53** **closed** on `main`; **M54** **active**; **M55**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M53** — Replay↔execution equivalence evidence surface v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M54** — Replay↔execution equivalence audit & acceptance gates v1 — **active** (Phase VII; see §11). **M54** adds the first **audit / acceptance-gate** layer over **M53** evidence — **not** satisfied by evidence-only JSON alone; **not** universal equivalence. |
| Next planned follow-on | **Phase VI integrated test campaign** (operator-local; **not** default CI) — see `docs/diligence/phase_vi_integrated_test_campaign.md` — **not** claimed complete unless separately executed and evidenced. |
...
| Phase VII pipeline (compact) | **M52**–**M61** — trust, equivalence, benchmark integrity, live SC2-in-CI posture, ladder/public evaluation protocol, audit hardening, v1 release lock — see §6; **M52** ledger + equivalence **charter** **closed** on `main`; **M53** equivalence **evidence** surface **closed** on `main` (bounded profiles); **M54** **active** (audit + acceptance gates over M53 evidence); **M55**–**M61** **stub/planned** until chartered/closed. |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M54**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
Future rows remain **stub/planned** until implemented — do not treat as satisfied by default.
...
- **M54** — Replay↔execution equivalence audit & acceptance gates v1 (**active** — consumes M53 evidence; deterministic audit + gate pack; **not** universal equivalence).
- **M55** — Benchmark integrity charter & split-governance controls v1 (**stub/planned**).
- **M56** — Benchmark integrity evidence & reproducibility gates v1 (**stub/planned**).
- **M57** — Narrow live SC2 in CI charter & controlled runner v1 (**stub/planned**).
- **M58** — Live SC2 in CI hardening & cost guardrails v1 (**stub/planned**).
- **M59** — Ladder/public evaluation protocol & evidence surface v1 (**stub/planned**).
- **M60** — Audit hardening & v2 readiness v1 (**stub/planned**) — targeted structural/CI debt **relevant to v2 readiness**, not generalized cleanup for its own sake (historical anchor: **M32** full-audit pressure points such as cross-layer coupling and large modules — revisit what remains material by then).
- **M61** — SC2 foundation release lock & v1 proof pack (**stub/planned**).

---

## 7. Milestone table

Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M53** **closed** on `main`; **M54** **active**; **M55**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M54 | Replay↔Execution Equivalence Audit & Acceptance Gates v1 | VII | Active | — | — |
| M55 | Benchmark Integrity Charter & Split-Governance Controls v1 | VII | Stub | — | — |
| M56 | Benchmark Integrity Evidence & Reproducibility Gates v1 | VII | Stub | — | — |
| M57 | Narrow Live SC2 in CI Charter & Controlled Runner v1 | VII | Stub | — | — |
| M58 | Live SC2 in CI Hardening & Cost Guardrails v1 | VII | Stub | — | — |
| M59 | Ladder/Public Evaluation Protocol & Evidence Surface v1 | VII | Stub | — | — |
| M60 | Audit Hardening & v2 Readiness v1 | VII | Stub | — | — |
| M61 | SC2 Foundation Release Lock & v1 Proof Pack | VII | Stub | — | — |
...
**M54 note:** **M54** is **active** (current milestone). **Narrow proof target:** deterministic audit + report JSON over **M53** evidence; gate pack **`starlab.m54.gatepack.identity_binding_acceptance_v1`**; structured `allowed_absence_policy`; profile-scoped `profile_scope_status` + descriptive `merge_bar_language` — **not** universal equivalence, **not** benchmark integrity, **not** live SC2 in CI, **not** repository branch protection. **Closeout** (tag / §18 row) waits on merge to `main` + CI evidence per workflow discipline.
...
| M54 | Replay↔execution equivalence audit & acceptance gates v1 — **active** (deterministic audit over M53 evidence; first gate pack; **not** universal equivalence) |
| M55 | Benchmark integrity charter & split-governance controls v1 — **stub/planned** |
| M56 | Benchmark integrity evidence & reproducibility gates v1 — **stub/planned** |
| M57 | Narrow live SC2 in CI charter & controlled runner v1 — **stub/planned** |
| M58 | Live SC2 in CI hardening & cost guardrails v1 — **stub/planned** |
| M59 | Ladder/public evaluation protocol & evidence surface v1 — **stub/planned** |
| M60 | Audit hardening & v2 readiness v1 — **stub/planned** (targeted; not vague cleanup) |
| M61 | SC2 foundation release lock & v1 proof pack — **stub/planned** |
...
### M54 — Replay↔execution equivalence audit & acceptance gates v1 — **active**

**Status:** current milestone pointer (**Phase VII**). **Product (in repo):** runtime `docs/runtime/replay_execution_equivalence_audit_acceptance_gates_v1.md`; `python -m starlab.equivalence.emit_replay_execution_equivalence_audit` with `--evidence` (required), optional `--evidence-report`; deterministic `replay_execution_equivalence_audit.json` + `replay_execution_equivalence_audit_report.json`; gate pack **`starlab.m54.gatepack.identity_binding_acceptance_v1`** for **`starlab.m53.profile.identity_binding_v1`** — **consumes M53 evidence JSON** (canonical SHA-256 of the evidence object), **does not** rebuild evidence from raw artifacts. **Explicit non-claims:** profile-scoped acceptance only; **not** universal replay↔execution equivalence; **not** benchmark integrity; **not** live SC2 in CI; **not** ladder/public performance; **`merge_bar_language`** is descriptive (not branch protection). Folder: `docs/company_secrets/milestones/M54/`.
...
**M51**–**M53** are **closed** on `main`; **M54**–**M61** (**Phase VII**) are **the remaining planned v1 proof track** — **not** implied solved because they appear in the milestone table. **M52** delivered **governance + charter** only; **M53** delivered **evidence surface** JSON/report only (evidence-only posture); **M54** is **active** as the **audit / acceptance-gate** layer over M53 evidence; **M55**–**M61** remain **stub/planned** until chartered/closed.
...
### 2026-04-15 — **M54** Replay↔execution equivalence audit & acceptance gates v1 — **active** (implementation branch)

- **Product (narrow):** runtime `docs/runtime/replay_execution_equivalence_audit_acceptance_gates_v1.md`; `starlab.equivalence` deterministic `replay_execution_equivalence_audit.json` / `replay_execution_equivalence_audit_report.json`; gate pack **`starlab.m54.gatepack.identity_binding_acceptance_v1`** over M53 **`starlab.m53.profile.identity_binding_v1`** evidence; CLI `python -m starlab.equivalence.emit_replay_execution_equivalence_audit` — **consumes** M53 evidence (canonical SHA-256), optional M53 report cross-check — **not** universal replay↔execution equivalence; **not** benchmark integrity; **not** live SC2 in CI; **not** ladder/public performance; **not** GitHub branch protection.
- **Ledger:** §1 / quick scan / Phase VII profile + gatepack tables / §6–§8 / §7 (**M54** **Active**) / §11 / §23; **M53** remains **closed** and evidence-only.
