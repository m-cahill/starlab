...
| Last closed milestone | **M43** — Hierarchical Training Pipeline v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M44** — Local Live-Play Validation Harness v1 (**in progress**; Phase VI; see §11) — **not** claimed **closed** on `main` until merge with authoritative CI. |
| Next planned training milestone | **M45** — Self-Play / RL Bootstrap v1 (**planned**; **not** claimed until closed on `main` with authoritative CI). |
...
| Phase VI local vs CI | Flat training runs **`out/training_runs/`** (M41); hierarchical runs **`out/hierarchical_training_runs/`** (M43); comparison **`out/comparisons/`** (M42); live validation **`out/live_validation_runs/`** (M44). All **local-first**. CI validates **fixture-only** CPU paths (**no** GPU training; **no** live SC2). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M44** **closed** product on `main` (until merge + authoritative CI), **M45+** self-play / RL **product** results (until each milestone closes on `main`), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
| M44 | Local Live-Play Validation Harness v1 | VI | In progress | v0.0.44-m44 | — |
...
| M44 | Local live-play validation harness v1 — replay-backed validation for an **M43** hierarchical candidate (**in progress** on-branch; **not** claimed **closed** until merge on `main`) |
...
| **M44** | **Local live-play validation harness v1** — `starlab.sc2`, M43 candidate + local `joblib`, replay binding, optional video metadata; **in progress** on-branch (**not** claimed **closed** until merge on `main`; see §11). |
| **M45** | **Self-play / RL bootstrap v1** — **stub** until closed on `main` (see §11). |
...
**M40**–**M43** are **closed** on `main`. Until **M44**–**M45** each **merge and close** on `main` with recorded CI, treat the following as **not proved** by default:

- **M44**–**M45** Phase VI **training-track product** milestones (**not** claimed until each closes with recorded CI); **M42** comparison artifacts **do not** prove benchmark integrity, statistical significance, or live-play outcomes.
