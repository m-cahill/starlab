...
| Current milestone | **`V15-M05`** — *Strong-Agent Benchmark Protocol* — **in progress** (contract `starlab.v15.strong_agent_scorecard.v1` + profile `starlab.v15.strong_agent_benchmark_protocol.v1`; runtime **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; emitter `python -m starlab.v15.emit_v15_strong_agent_scorecard` — **protocol / scorecard contract only**; **not** executed benchmark; **not** strong-agent claim). **Last closed (v1.5):** **`V15-M04`** on `main` ([PR #123](https://github.com/m-cahill/starlab/pull/123); contract `starlab.v15.xai_evidence_pack.v1`). **Authority** **`docs/starlab-v1.5.md`**. M04 (closed) **`docs/runtime/v15_xai_evidence_contract_v1.md`**; M03 **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02 **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** perform a **long wall-clock GPU training session** as a program deliverable. Default **CI** remains **CPU** fixture self-play / bootstrap paths. **`PX2-M04`** / **`PX2-M05`** — **planned**; **`PX1-M05`** optional; **v2** not opened. |
...
| Next planned follow-on | **`V15-M05`** — *Strong-Agent Benchmark Protocol* — **in progress** (see **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; **not** benchmark execution). **v1.5** authority: **`docs/starlab-v1.5.md`**. M04 XAI contract (closed): **`docs/runtime/v15_xai_evidence_contract_v1.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only** on the **PX2** namespace. **`PX1-M05`** optional. **`docs/starlab-vision.md`** **v2** directions remain **outside** the closed **M00–M61** arc until **explicit** recharter — **v2** is **deferred** / **not** opened. |
...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M04`** **closed** on `main` (PR #116–#120, [PR #123](https://github.com/m-cahill/starlab/pull/123)). **`V15-M05`** (strong-agent **protocol** contract) is **in progress** — **not** executed benchmark, **not** strong-agent **claim** (see `docs/starlab-v1.5.md`). Checkpoint lineage **manifest** is **metadata** — **not** training **execution**, **not** byte verification by default. XAI pack contract **`starlab.v15.xai_evidence_pack.v1`** is **closed** (fixture + schema; **not** real XAI inference or faithfulness proof). Registers + M01–M04 **fixture**-safe contracts; **not** long GPU **training**; **not** claim-critical asset approval. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI demo** + (full program) data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not satisfied by **PX2** alone | **V15-M06+** (after M05 **protocol** closure) |
...
**Current (v1.5):** **`V15-M05`** — *Strong-Agent Benchmark Protocol* — **in progress** (runtime **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; emitter `python -m starlab.v15.emit_v15_strong_agent_scorecard`). **Closed v1.5 line on `main`:** **`V15-M00`**, **`V15-M01`**, **`V15-M02`**, **`V15-M03`**, **`V15-M04`**. **Authority:** **`docs/starlab-v1.5.md`**. **Prior** M04: **`docs/runtime/v15_xai_evidence_contract_v1.md`**; M03: **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**.

### V15-M05 — *Strong-Agent Benchmark Protocol* — **in progress**

**Intent:** Deterministic **strong-agent scorecard / benchmark protocol** JSON + report (`starlab.v15.strong_agent_scorecard.v1` + `starlab.v15.strong_agent_benchmark_protocol.v1`); default **`fixture_ci`**; optional **`operator_declared`** with **`--protocol-json`**. **Not** benchmark execution; **not** live SC2; **not** checkpoint evaluation; **not** strong-agent certification; see **`docs/starlab-v1.5.md`** M05 non-claims.
