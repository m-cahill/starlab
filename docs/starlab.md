...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M56** **closed** on `main` — benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) — **not** global benchmark integrity proof; **M57**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M56** — Benchmark integrity evidence & reproducibility gates v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M57** — Narrow live SC2 in CI charter & controlled runner v1 — **stub/planned** (Phase VII; next after **M56** **closed** on `main`). **M56** delivered **bounded** evidence + reproducibility gates for one fixture-only offline chain — **not** global benchmark integrity proof. **Not** replay↔execution equivalence (that track is **M52**–**M54**, **closed** on `main`). |
...
- **M57** — Narrow live SC2 in CI charter & controlled runner v1 (**stub/planned** — current milestone pointer after **M56** **closed** on `main`).
...
**Status:** current milestone pointer (**Phase VII**). **Depends on:** **M56** **closed** on `main` — bounded benchmark-integrity evidence + reproducibility gates — **not** global benchmark integrity proved.

**Explicit non-claims (until chartered):** **no** live SC2-in-CI proof, **no** ladder/public evaluation, **no** merge-bar automation — **M57** is **not** started as product work in this ledger row. Folder: `docs/company_secrets/milestones/M57/` (stub).
