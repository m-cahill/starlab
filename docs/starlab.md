...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M57** **closed** on `main`; **M58** live-SC2 **hardening + guardrails** **implemented** in repo — **not** global live-SC2 merge norms; **M59**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M57** — Narrow live SC2 in CI charter & controlled runner v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M58** — Live SC2 in CI hardening & cost guardrails v1 — **active** (Phase VII — guardrails JSON, preflight receipt, hardened optional workflow; still **not** default merge-gate live SC2). **M56** **closed** — bounded benchmark-integrity evidence + gates — **not** global proof. **Not** replay↔execution equivalence (**M52**–**M54** **closed**). |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI as default merge-gate / global proof**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M58**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
- **M58** — Live SC2 in CI hardening & cost guardrails v1 (**implemented** — runtime + guardrails JSON + preflight receipt + hardened manual workflow; **not** merge-gate live SC2 — follows **M57**).
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M57** **closed** on `main` (through benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) + narrow live-SC2-in-CI **charter + controlled runner** (**M57**) milestones); **M58** guardrails/hardening surface **implemented** (tag **`v0.0.58-m58`** pending milestone closeout); **M59**–**M61** stub/planned; **M51** **closed** on `main`):
...
### M58 — Live SC2 in CI hardening & cost guardrails v1 — **active / implemented (product)**

**Status:** current milestone pointer (**Phase VII**). **Depends on:** **M57** **closed** on `main`. **M58** hardens that bounded surface — **not** default merge-gate live SC2; **not** global live-SC2-in-CI proof; **not** ladder/public evaluation (**M59**).
...
**Explicit non-claims:** **no** new live runner profiles beyond **`starlab.m57.runner_profile.m44_single_validation_v1`**; **no** new candidate classes beyond bounded **M43** + explicit weights; **no** substitution for **M52**–**M54** / **M55**–**M56** tracks. Folder: `docs/company_secrets/milestones/M58/`.

### M59 — Ladder/public evaluation protocol & evidence surface v1 — **stub/planned**

**Intent (stub):** next Phase VII track after **M58** — **not** satisfied by **M58** guardrail JSON or preflight alone.
