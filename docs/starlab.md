...
| Last closed milestone | **M42** — Learned-Agent Comparison Harness v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M43** — Hierarchical Training Pipeline v1 (**stub**; Phase VI; see §11). |
| Next planned training milestone | **M44** — Local Live-Play Validation Harness v1 (**planned**; **not** claimed until closed on `main` with authoritative CI). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M43+** hierarchical / live-play / self-play **product** results (until each milestone closes on `main`), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
- hierarchical training pipeline (**M43** — **current stub**)
- local live-play validation harness (**M44** — local-only; not CI-proved)
- self-play / RL bootstrap (**M45**)

**Phase VI artifact family (compact):** **M40** — training-program **contract** artifacts (`agent_training_program_contract.json` / report); **M41** — **training run** artifacts (`replay_imitation_training_run.json` / report + optional local weights); **M42** — **comparison** artifacts (`learned_agent_comparison.json` / report) — all **closed** on `main`. Training and comparison remain **local-first**; CI stays **fixture-only** (no GPU training, no live SC2, no weights in-repo).
...
| M43 | Hierarchical Training Pipeline v1 | VI | Stub | v0.0.43-m43 | — |
...
| M43 | Hierarchical training pipeline v1 (**stub**) |
...
| **M43** | **Hierarchical training pipeline v1** (stub; **not** claimed until closed on `main`). |
...
### M43 — Hierarchical Training Pipeline v1 — **stub** (current)

**Planned.** Not claimed until implemented and closed on `main` with authoritative CI.
...
**M40**–**M42** are **closed** on `main`. Until **M43**–**M45** each **merge and close** on `main` with recorded CI, treat the following as **not proved** by default:

- **M43**–**M45** Phase VI **training-track product** milestones (**not** claimed until each closes with recorded CI); **M42** comparison artifacts **do not** prove benchmark integrity, statistical significance, or live-play outcomes.
...
- §11 **current milestone** → **M43** (**stub**); **M42** **Complete**; closeout `M42_run1.md`, `M42_summary.md`, `M42_audit.md`, `M42_plan.md` (**Complete**), `M42_toolcalls.md`.
