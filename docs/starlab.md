...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M60** **closed** on `main`; **M61** **in progress**). |
| Last closed milestone | **M60** — Audit hardening & v2 readiness v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M61** — SC2 foundation release lock & v1 proof pack — **in progress** (proof pack + audit emitters in-repo; operator-local campaign release evidence **planned** — see §11). **M60** **closed** — structural/diligence (private campaign executor split + guardrails + audit mapping) — **not** new governed product artifact family; **not** equivalence/benchmark/ladder/live-SC2 claims. **M59** **closed** — bounded descriptive ladder/public protocol + evidence — **not** ladder/public **performance** proof; **not** benchmark integrity; **not** replay↔execution equivalence (**M52**–**M54** **closed**). **M58** **closed** — bounded **M57** hardening + cost guardrails — **not** global live-SC2-in-CI proof. |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI as default merge-gate / global proof**, **ladder / public performance**, **Phase VI integrated test campaign** **as fully evidenced M61 release-lock closure** (operator-local packaging TBD until executed), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. The **remaining v1 foundation-completion arc** is **M61** only — **in progress** — not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
**M61 operator-local closure** (`release_scope_status`: `ready_within_scope`) remains **planned** until an operator-designated campaign is executed and captured **without** committing raw `out/` secrets to the repo.
...
- **M61** — SC2 foundation release lock & v1 proof pack (**in progress** — proof pack + audit machinery in repo; operator-local release evidence **planned**).

---

## 7. Milestone table

Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M60** **closed** on `main` (through benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) + narrow live-SC2-in-CI **charter + controlled runner** (**M57**) + **M58** guardrails/hardening + **M59** ladder/public descriptive protocol/evidence + **M60** audit hardening milestones); tag **`v0.0.60-m60`** on **M60** merge commit; **M61** **in progress**; **M51** **closed** on `main`):
...
| M61 | SC2 foundation release lock & v1 proof pack — **in progress** (release-lock emitters + docs; operator campaign closure **planned**) |
...
- **Closeout:** `M60_run1.md`, `M60_summary.md`, `M60_audit.md` (**local only** under `docs/company_secrets/` — **not** committed); §1 / quick scan / §6 / §7 **M60** **Complete** + tag **`v0.0.60-m60`** / §11 current milestone pointer **M61** (**stub/planned**) + **M60** **closed** section / §18 / §23; `docs/runtime/v2_readiness_audit_hardening_v1.md` header (**M60** **closed** on `main`); governance tests (`tests/test_m58_live_sc2_in_ci_hardening_cost_guardrails.py`) ledger pointer checks.
