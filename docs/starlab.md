...
**M29 note:** **M29** adds **hierarchical agent interface** — deterministic `hierarchical_agent_interface_schema.json` + `hierarchical_agent_interface_schema_report.json`; runtime contract `docs/runtime/hierarchical_agent_interface_v1.md`, modules + CLI under `starlab/hierarchy/`; **no** `starlab.replays`, **no** `starlab.sc2`, **no** `s2protocol` in listed M29 hierarchy modules — see §6 Phase V artifact row. **Merge PR + `main` CI merge authority pending** (record in §18 / §23 on merge). **Not** learned hierarchical agent (**M30**), **not** benchmark integrity, **not** live SC2 in CI.
...
| Hierarchical agent interface (Phase V; contract schema) | M29 | **Branch** `m29-hierarchical-agent-interface-layer` — narrow `hierarchical_agent_interface_schema.json` + report; runtime `docs/runtime/hierarchical_agent_interface_v1.md`, `starlab/hierarchy/` — **merge PR + `main` CI merge authority pending** (record in §18 / §23); **not** learned hierarchical agent (**M30**), **not** benchmark integrity, **not** live SC2 in CI |
...
| **M29** | **Hierarchical agent interface layer** — deterministic `hierarchical_agent_interface_schema.json` / `hierarchical_agent_interface_schema_report.json` (contract-first; **merge to `main` pending**). |
...
**Status:** **Current** milestone — **M30** (**stub-only** for product code — see `docs/company_secrets/milestones/M30/M30_plan.md`). **M29** is **closed** on branch `m29-hierarchical-agent-interface-layer` (**merge PR + `main` CI merge authority pending** — record in §18 / §23 on merge); plan `docs/company_secrets/milestones/M29/M29_plan.md` (**Complete**).
...
| M29       | pending merge | pending | pending | Hierarchical agent interface (`hierarchical_agent_interface_schema.json` / `hierarchical_agent_interface_schema_report.json`); **branch** `m29-hierarchical-agent-interface-layer` — **record PR #, merge commit, and CI in this row + §23 on merge** |
...
**M29 merge:** **pending** — record [PR #NN](https://github.com/m-cahill/starlab/pull/NN), merge commit, and CI when merged to `main` (see §23 changelog entry **2026-04-09 — M29 hierarchical agent interface layer**).

**M29 CI evidence (PR-head run — authoritative merge gate):** **pending**

**M29 CI evidence (post-merge `main` — merge boundary):** **pending**
...
**M29 note:** Evidence column reflects **hierarchical agent interface** (`hierarchical_agent_interface_schema.json` / `hierarchical_agent_interface_schema_report.json`) — **branch** `m29-hierarchical-agent-interface-layer`; **merge PR + `main` CI merge authority pending** (record in §18 / §23 on merge). **Not** learned hierarchical agent (**M30**), **not** benchmark integrity, **not** live SC2 in CI, **not** `starlab.replays` / `starlab.sc2` / `s2protocol` in listed M29 hierarchy modules.
...
### 2026-04-09 — M29 hierarchical agent interface layer (branch `m29-hierarchical-agent-interface-layer`)

- **Branch:** `m29-hierarchical-agent-interface-layer` — **merge PR + authoritative PR-head / merge-boundary `main` CI — pending** (record PR #, merge commit, and workflow run IDs in §18 / this §23 on merge)
- Product: `docs/runtime/hierarchical_agent_interface_v1.md`, `starlab/hierarchy/` (`hierarchical_interface_models.py`, `hierarchical_interface_schema.py`, `hierarchical_interface_io.py`, `emit_hierarchical_agent_interface.py`), `tests/fixtures/m29/`, `tests/test_hierarchical_agent_interface.py`; **513** pytest locally (one pre-existing `s2protocol` deprecation warning in replay CLI tests — unchanged)
...
- §3 / §6 / §7 / §8 / §10 / §11 / §18 / §20 / §23: **M29** **Complete** (product on branch; **merge authority pending**); **current milestone** → **M30** (**stub-only**); **M30** stubs (`M30_plan.md`, `M30_toolcalls.md`)
