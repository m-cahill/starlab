...
**v1.5 arc (V15) — authoritative public governance:** `docs/starlab-v1.5.md` (strategic anchor: `docs/starlab-v1.5moonshot.md`)
...
| Current milestone | **`V15-M01`** — *Training-Scale Provenance and Asset Registers* — **active** (public register templates + `starlab.v15.training_asset_registers.v1`); **not** GPU training execution — see **`docs/starlab-v1.5.md`**. **Last closed (v1.5):** **`V15-M00`** on `main` ([PR #116](https://github.com/m-cahill/starlab/pull/116)). **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** perform a **long wall-clock GPU training session** as a program deliverable. Default **CI** remains **CPU** fixture self-play / bootstrap paths. **`PX2-M04`** / **`PX2-M05`** — **planned**; **`PX1-M05`** optional; **v2** not opened. |
...
| Next planned follow-on | **`V15-M01`** (provenance / asset registers) — **active**; **`V15-M02`** environment lock next in roadmap. **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only** on the **PX2** namespace. **`PX1-M05`** optional. **`docs/starlab-vision.md`** **v2** directions remain **outside** the closed **M00–M61** arc until **explicit** recharter — **v2** is **deferred** / **not** opened. |
...
| **v1.5 (V15)** | **`V15-M00` closed** on `main`; **`V15-M01` active** (register surfaces + `starlab.v15.training_asset_registers.v1`) — **not** long GPU run; **not** claim-critical asset approval. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI** + checkpoint/data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not in **PX2** | **V15-M01**+ |
...
**Ledger truths (PX2 phase, **closeout**; program `current milestone` = **`V15-M00`** / **v1.5**):**
...
**Current (v1.5):** **`V15-M01`** — *Training-Scale Provenance and Asset Registers* — **active** (public register templates + deterministic register contract JSON); **does not** execute GPU training or register claim-critical assets — **`docs/starlab-v1.5.md`**, **`docs/runtime/v15_training_scale_provenance_asset_registers_v1.md`**.

**Last closed (v1.5):** **`V15-M00`** — v1.5 **Training Readiness Charter and Long GPU Run Gate** — **closed** on `main` ([PR #116](https://github.com/m-cahill/starlab/pull/116); merge `1391518e…`; **authoritative PR-head CI** [`24911939851`](https://github.com/m-cahill/starlab/actions/runs/24911939851); **merge-boundary `main` CI** [`24913025515`](https://github.com/m-cahill/starlab/actions/runs/24913025515) — **success**). **Authoritative public detail:** **`docs/starlab-v1.5.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** run a **long wall-clock GPU** training **campaign** as a program deliverable. **PX2-M03** readiness (historical): **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**; runtime **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** (slices + §8q). **Last closed PX2 milestone:** **`PX2-M03`** — prior **`PX2-M02`** ([PR #96](https://github.com/m-cahill/starlab/pull/96)). **`PX2-M04`** / **`PX2-M05`** — **planned only**. **`PX1-M05`** **optional**; **v2** **not** opened.

### V15-M01 — Training-Scale Provenance and Asset Registers — **active**

**Intent:** Public **template** registers and **`starlab.v15.training_asset_registers.v1`** emission (`python -m starlab.v15.emit_v15_training_asset_registers`) — **not** long GPU execution, **not** asset approval for claim-critical use, **not** **PX2-M04** / **PX2-M05**.

**Non-claims (preserved):** No benchmark or human-panel **execution** in M01; no XAI contract freeze (**V15-M04**); no checkpoint lineage **runtime** (**V15-M03**).
