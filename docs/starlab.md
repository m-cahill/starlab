...
**Governance recharter (2026-04-13 — user-directed):** The ledger stub **M47 — Learned-agent comparison contract-path alignment** is **deferred** to **M48** (stub). **M47** is rechartered to **Bootstrap Episode Distinctness & Operator Ergonomics** — M45 multi-episode **interpretation** rules, **per-episode** M02 `seed` (`bootstrap_base_seed + episode_index`), **`starlab.m47.episode_manifest.v2`**, distinctness/collapse reporting, and operator guidance — **not** benchmark integrity, **not** M42 `--contract` product work (see **M48**).

**M47 — Bootstrap Episode Distinctness & Operator Ergonomics:** **closed** on `main` ([PR #58](https://github.com/m-cahill/starlab/pull/58); merge commit `ebc5de0864ef6231d13efa741150d73c1ef1b98b`; merged **2026-04-14T00:47:56Z** (UTC); **final PR head** `4a8fb3e2f7aad95d2cde5b6b77577db25e42e91e`; **authoritative PR-head CI** [`24374720293`](https://github.com/m-cahill/starlab/actions/runs/24374720293); **merge-boundary `main` CI** [`24374756823`](https://github.com/m-cahill/starlab/actions/runs/24374756823) on merge commit `ebc5de0…` — **success**; tag **`v0.0.47-m47`**; branch `m47-bootstrap-episode-distinctness-ergonomics` **retained** on `origin`; see §18 / `M47_run1.md`). **Narrow:** per-episode M02 **`seed`**, **`starlab.m47.episode_manifest.v2`**, **`episode_distinctness`** / collapse **warnings**, operator/runtime interpretation — **not** benchmark integrity; **M42** contract-path alignment is **M48** (see below).

**M48 — Learned-agent comparison contract-path alignment:** **in progress** on branch `m48-learned-agent-comparison-contract-path-alignment` (**not** merged to `main` until PR closeout). **Narrow:** explicit **`--benchmark-contract`** (M20) vs **`--training-program-contract`** (M40); strict M41 charter identity check; see `docs/runtime/learned_agent_comparison_harness_v1.md`, `docs/company_secrets/milestones/M48/M48_plan.md`.
...
| ----- | ------------------------ |
| Program arc | **49 milestones (M00–M48)** — see §7. |
| Last closed milestone | **M47** — Bootstrap Episode Distinctness & Operator Ergonomics (merged to `main`; see §11 / §18). |
| Current milestone | **M48** — **M42** contract-path alignment — **in progress** on branch `m48-learned-agent-comparison-contract-path-alignment` (**not** merged to `main` until PR). |
...
| Phase VI pipeline (compact) | **M40** contract → **M41** flat training run → **M42** comparison → **M43** hierarchical training → **M44** local live-play → **M45** self-play / RL → **M46** bounded live `final_status` / `sc2_game_result` semantics → **M47** bootstrap episode distinctness / operator ergonomics → **M48** M42 contract-path alignment (**in progress** on branch) |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
| M48 | Learned-Agent Comparison Contract-Path Alignment | VI | In progress (branch) | — | — |
...
| M48 | Align **M42** benchmark vs **M40** charter CLI paths + M41 identity check — **in progress** on branch; **not** bounded live semantics |
...
### M48 — Learned-Agent Comparison Contract-Path Alignment — **in progress** (current)

**Governance recharter (2026-04-13 — user-directed):** The prior ledger **M47** stub (**M42** contract-path alignment vs **M40** / **M20**) is **deferred** to **M48**. **In progress** on branch `m48-learned-agent-comparison-contract-path-alignment` (**not** merged to `main` until PR).

**Scope (narrow):** **M20** benchmark contract via **`--benchmark-contract`** (alias **`--contract`**); optional **`--training-program-contract`** for on-disk M40 JSON (default: in-process `build_agent_training_program_contract()`); when **`--m41`** candidates are present, **strict** **`ValueError`** if any M41 run’s `training_program_contract_sha256` / `training_program_contract_version` does not match the active M40 charter. **Not** benchmark integrity, **not** new M28 metrics, **not** live SC2 in CI. Runtime: `docs/runtime/learned_agent_comparison_harness_v1.md`; plan: `docs/company_secrets/milestones/M48/M48_plan.md`.

### M47 — Bootstrap Episode Distinctness & Operator Ergonomics — **closed**

**Governance recharter (2026-04-13 — user-directed):** The prior ledger **M47** stub (**M42** `--contract` path alignment) is **deferred** to **M48** (stub). **M47** is rechartered to **Bootstrap Episode Distinctness & Operator Ergonomics**.
...
| OD-007 | Second-environment posture       | Deferred | Beyond active arc (M00–M45) | Formerly tied to legacy **M41** platform charter stub; deferred until a future recharter after the training track — see §19 |
