...
| Structured state pipeline (bundle → one frame) | M16 | **On `main`** — narrow M14 bundle load + deterministic derivation + schema validation + report; not observation surface, tensors, or replay↔execution equivalence |
...
| **M16 structured state pipeline** | **Proved on `main`:** **materialize** exactly one M15-shaped `canonical_state.json` per CLI invocation from a **complete M14 bundle** + governed upstream JSON, with `jsonschema` validation and `canonical_state_report.json` — **not** an observation API, **not** tensors or action masks (**M17**). |
| **M17 observation surface contract** | **Planned** milestone for **agent-facing observation** semantics (tensors, masks, contracts) — **distinct** from M15 schema-only proof and M16 bundle→frame pipeline mechanics. |
...
**Status:** **Planned** — **current** milestone; **M16** is **closed** on `main` (see §18). **Phase III** is **in progress** (M15–M16 complete; M17–M19 planned).

**Goal (high level):** Define the **agent-facing observation surface** (contracts, semantics) — **not** proved in M15 (schema-only) or M16 (bundle→frame pipeline); see Phase III artifact table (§6) and Phase III glossary (§10).

**Primary references:** `docs/company_secrets/milestones/M17/M17_plan.md` (stub); `docs/starlab.md` §10 (proved vs not yet proved).

**Note:** **M16** proves **governed structured state materialization** — `canonical_state.json` / `canonical_state_report.json` from an M14 bundle at one `gameloop`, with M15 schema validation — **not** observation tensors, **not** action masks, **not** a perceptual bridge.

#### Current milestone — explicit non-claims (standing)

Until **M17** explicitly closes a claim, treat the following as **not proved** for **M17** planning:
