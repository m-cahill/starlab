...
| M20       | TBD           | [#21](https://github.com/m-cahill/starlab/pull/21) | TBD | Benchmark contract + scorecard schemas (`benchmark_contract_schema.json` / `benchmark_contract_schema_report.json` / `benchmark_scorecard_schema.json` / `benchmark_scorecard_schema_report.json`); CI evidence TBD — see §18 / `M20_run1.md` |
...
**M20 note:** Evidence column reflects **benchmark contract + scorecard JSON Schema emission** + fixture validation + full governance CI **on `main`** (PR #21 — CI rows in §18 / `M20_run1.md`). **Not** scripted/heuristic baselines (**M21–M22**), evaluation runner (**M23**), benchmark integrity, replay↔execution equivalence, or live SC2 in CI.
...
- Merged [PR #21](https://github.com/m-cahill/starlab/pull/21) to `main` (merge commit and CI run IDs recorded in §18 / `M20_run1.md` when available).
- Product: `docs/runtime/benchmark_contract_scorecard_v1.md`, `starlab/benchmarks/` (contract models + JSON Schema builders + `python -m starlab.benchmarks.emit_benchmark_contracts`), `tests/fixtures/m20/`, `tests/test_benchmark_contracts.py`; Phase IV artifact row + Phase IV scorecard glossary (`scored` / `unscored` / `disqualified` / `comparable` / `provisional` / `non_comparable`).
- §3 / §6 / §10 / §11 / §18 / §20 / §23: **governed benchmark contract + scorecard schemas** (narrow) **proved on `main`**; **current milestone** → **M21** (stub); `docs/company_secrets/milestones/M21/` seeded (`M21_plan.md`, `M21_toolcalls.md` only).
