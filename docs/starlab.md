...
**Phase IV boundary (M20–M23):** **M20** defines benchmark contract + scorecard schemas. **M21** and **M22** are **fixture-only baseline suite emitters** (scripted vs heuristic subjects) until **M23** introduces evaluation runner / tournament harness semantics — not proved in M21–M22.
...
| Heuristic baseline suite (fixture-only) | M22 | **Narrow proof** — heuristic suite + report + embedded scorecards; **merge + CI to be recorded** in §18 / `M22_run1.md` with the landing PR; **not** runner (**M23**), benchmark integrity, or replay↔execution equivalence |
...
| **M22** | **Heuristic baseline suite** — same contract posture as M21 with fixed **heuristic** subjects (`heuristic_baseline_suite.json` / `heuristic_baseline_suite_report.json`); **fixture-only scorecard emitter** until **M23** adds runner semantics (merge + CI **to be recorded** in §18 with the landing PR). |
...
**Status:** **Planned** — **current** milestone; **M22** heuristic baseline suite **product** is implemented (`docs/runtime/heuristic_baseline_suite_v1.md`, `starlab/baselines/`); **merge + authoritative CI** for the landing PR **to be recorded** in §18 / `M22_run1.md`. **M23** remains **stub-only** (`docs/company_secrets/milestones/M23/`) until authorized — **no** M23 product code in the M22 milestone.

**Goal (high level):** **M23** introduces an **evaluation runner** and **tournament harness** — **not** part of **M21**–**M22** fixture-only baseline suite emitters.

**Primary references:** `docs/company_secrets/milestones/M23/M23_plan.md`; `docs/starlab.md` §6 Phase IV boundary (M20–M23).

**Note:** **M21** and **M22** prove **fixture-only** scripted and heuristic baseline **suite + embedded scorecard emission** — **not** runner semantics, **not** tournament harness, **not** benchmark integrity, **not** live SC2 in CI.

#### Current milestone — explicit non-claims (standing)

Until **M23** explicitly closes a claim, treat the following as **not proved** for **M23** planning:
...
- **Evaluation runner** and **tournament harness** semantics (**M23** — not started while stubs only).
...
| M22       | TBD           | TBD | TBD | Heuristic baseline suite (`heuristic_baseline_suite.json` / `heuristic_baseline_suite_report.json`); **PR / merge / CI to be recorded** after merge to `main`; see §18 / `M22_run1.md` |
...
**M22 merge:** TBD — record PR number, merge commit, **authoritative green PR-head CI**, and **merge-boundary post-merge `main` CI** after merge to `main`; align `M22_run1.md` and this table.
...
**M22 note:** Evidence column reflects **heuristic baseline suite + report emission** from one **`fixture_only`** M20-validated benchmark contract + embedded M20 scorecards for **heuristic** subjects + full governance CI **on `main`** (**PR-head / merge-push runs to be recorded** in §18 / `M22_run1.md` after merge). **Not** evaluation runner (**M23**), tournament harness, benchmark integrity, replay↔execution equivalence, or live SC2 in CI.
...
### 2026-04-08 — M22 heuristic baseline suite (implementation branch; merge TBD)

- **Product:** `docs/runtime/heuristic_baseline_suite_v1.md`; `starlab/baselines/heuristic_baseline_models.py`, `heuristic_baseline_suite.py`, `heuristic_baseline_scorecards.py`, `emit_heuristic_baseline_suite.py`; goldens `tests/fixtures/m22/` (reuses `tests/fixtures/m21/valid_benchmark_contract.json`); `tests/test_heuristic_baseline_suite.py`
- **§6 / §7 / §10 / §11 / §18 / §20:** Phase IV **M22** row; **M21**–**M22** fixture-only baseline emitters vs **M23** runner boundary; **current milestone** → **M23** (stub); closeout ledger **M22** row **TBD** (fill PR/CI after merge); score trend **M22** row
- **Milestone artifacts:** `docs/company_secrets/milestones/M22/` (`M22_plan.md`, `M22_toolcalls.md`, `M22_run1.md`, `M22_summary.md`, `M22_audit.md`); **M23** stubs (`M23_plan.md`, `M23_toolcalls.md`) — **no** M23 product code
- **After merge:** Record authoritative PR-head + merge-boundary CI in `M22_run1.md`, replace **TBD** in §18 table row, and align status line §3 if needed
