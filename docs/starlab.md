...
| Current milestone | **`V15-M08`** — *Long GPU Campaign Execution* — **implementation in progress** (`emit_v15_long_gpu_training_manifest`, `run_v15_long_gpu_campaign`, **`docs/runtime/v15_long_gpu_campaign_execution_v1.md`**); **no** operator long-run closeout on `main` yet; **`long_gpu_run_authorized` false** in **fixture** / public path. **Last closed (v1.5):** **`V15-M07`** (prior **`V15-M06`**) on `main` — M07: [PR #129](https://github.com/m-cahill/starlab/pull/129) — training-run **receipt** + `emit_v15_training_run_receipt`; **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**; **not** M08 long campaign. **Prior closed:** M06 human-panel **protocol** ([PR #127](https://github.com/m-cahill/starlab/pull/127)) — **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**; M05 **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; M04 **`docs/runtime/v15_xai_evidence_contract_v1.md`**; M03 **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02 **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**. Default **CI** remains **CPU** fixture self-play / bootstrap. **`PX2-M04`** / **`PX2-M05`** — **planned**; **v2** not opened. |
...
| Next planned follow-on | **`V15-M08`** — *Long GPU Campaign Execution* — **implementation in progress**; operator-local long-run **execution** and honest closeout follow merge + explicit authorization (see **`docs/starlab-v1.5.md`**). **M07 (closed on `main`):** training-run receipt + optional operator shakedown **tooling** — **not** the long campaign; public record: **tooling-only** closure (**`operator_local_short_gpu_not_run`** for this program snapshot). **M06 (closed):** human-panel **protocol** + **fixture** only. **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only**. **v2** **deferred**. |
...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M07`** **closed** on `main` (M07: [PR #129](https://github.com/m-cahill/starlab/pull/129); M06: [PR #127](https://github.com/m-cahill/starlab/pull/127)); **`V15-M08`** — *Long GPU Campaign Execution* — **implementation in progress**. M07: training-run **receipt** + shakedown **tooling** — **not** long campaign. M06: protocol + `fixture_ci` + optional **operator_declared** — **not** human-panel **execution**; see **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**. | Full GPU run + **strong-agent** eval + XAI + **human-panel** eval — **chartered in v1.5**, not satisfied by **PX2** alone | **M08+** (long run), then eval / human / XAI per roadmap |
...
**Current (v1.5):** **`V15-M08`** — *Long GPU Campaign Execution* — **implementation in progress** (`starlab.v15.long_gpu_training_manifest.v1`, `emit_v15_long_gpu_training_manifest`, `run_v15_long_gpu_campaign`); **no** merge-bar operator long-run **completed** claim until receipts exist. **Closed v1.5 line on `main`:** **`V15-M00`**, **`V15-M01`**, **`V15-M02`**, **`V15-M03`**, **`V15-M04`**, **`V15-M05`**, **`V15-M06`**, **`V15-M07`**. **Authority:** **`docs/starlab-v1.5.md`**. M08 runtime: **`docs/runtime/v15_long_gpu_campaign_execution_v1.md`**. M07 (closed) runtime: **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**, implementation [PR #129](https://github.com/m-cahill/starlab/pull/129) — **authoritative PR-head CI** [`24925848388`](https://github.com/m-cahill/starlab/actions/runs/24925848388); **merge-boundary `main` CI** [`24925929052`](https://github.com/m-cahill/starlab/actions/runs/24925929052) — **success**; **operator-local shakedown** status: **tooling-only** / **`operator_local_short_gpu_not_run`** (closeout host). M06 (closed): **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**; M05: **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; M04: **`docs/runtime/v15_xai_evidence_contract_v1.md`**; M03: **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**; M01: **`docs/runtime/v15_training_scale_provenance_asset_registers_v1.md`**.

### V15-M08 — *Long GPU Campaign Execution* — **implementation in progress**

**V15-M08** ships the **governed long-campaign surface** (preflight gates **A–G**, manifest + receipt scaffolding, **dual-guard** runner wrapping **M49–M51** `execute_full_local_training_campaign`). **Fixture / public path:** **`long_gpu_run_authorized` false**; **no** substitute for an operator-local long run. Operator execution requires **`--allow-operator-local-execution`** and **`--authorize-long-gpu-campaign`**, valid **`campaign_plan.json`**, and honest gate results (see **`docs/starlab-v1.5.md`** M08 non-claims block).
