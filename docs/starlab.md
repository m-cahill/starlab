...
| ----- | ------------------------ |
| Program arc | **46 milestones (M00–M45)** — see §7. |
| Last closed milestone | **M40** — Agent Training Program Charter & Artifact Contract (merged to `main`; see §11 / §18). |
| Current milestone | **M41** — Replay-Imitation Training Pipeline v1 (**in progress** on branch; Phase VI; see §11). |
| Next planned training milestone | **M42** — Learned-Agent Comparison Harness v1 (**planned**; **not** claimed until closed on `main` with authoritative CI). |
| Phase VI local vs CI | Training runs and weights are **local-first** (`out/training_runs/`). CI validates a **fixture-only** CPU training path (**no** GPU training; **no** live SC2). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M42+** comparison / hierarchical / live-play / self-play **product** results (and **M41** until closed on `main`), **operating manual v1** — see §11 *explicit non-claims*. |
...
- replay-imitation training pipeline (**M41** — **in progress** on branch: deterministic run/report JSON + local sklearn weights; CI fixture-only; **M42**+ next)
- learned-agent comparison harness (**M42**)
- hierarchical training pipeline (**M43**)
- local live-play validation harness (**M44** — local-only; not CI-proved)
- self-play / RL bootstrap (**M45**)

---

## 7. Milestone table

Planned program arc (46 milestones, M00–M45):
...
| M41 | Replay-Imitation Training Pipeline v1 | VI | In progress | v0.0.41-m41 | — |
| M42 | Learned-Agent Comparison Harness v1 | VI | Planned | v0.0.42-m42 | — |
| M43 | Hierarchical Training Pipeline v1 | VI | Planned | v0.0.43-m43 | — |
| M44 | Local Live-Play Validation Harness v1 | VI | Planned | v0.0.44-m44 | — |
| M45 | Self-Play / RL Bootstrap v1 | VI | Planned | v0.0.45-m45 | — |
...
| M42 | Learned-agent comparison harness v1 |
| M43 | Hierarchical training pipeline v1 |
| M44 | Local live-play validation harness v1 (local-only) |
| M45 | Self-play / RL bootstrap v1 |
...
| **M41** | **Replay-imitation training pipeline v1** — `starlab.imitation`, `replay_imitation_training_run.json` / report, optional local joblib weights; local-first vs CI fixture-only (**in progress** on branch; **not** closed on `main` until merge). |
| **M42** | **Learned-agent comparison harness v1** (planned). |
| **M43** | **Hierarchical training pipeline v1** (planned). |
| **M44** | **Local live-play validation harness v1** — local-only; not CI-proved (planned). |
| **M45** | **Self-play / RL bootstrap v1** (planned). |
...
### M41 — Replay-Imitation Training Pipeline v1 — **in progress** (current)

**Objective:** Deliver the first governed **local-first** replay-imitation **training run** surface: consume governed **M26** `replay_training_dataset.json` and **M14** bundle directories, train a deterministic **scikit-learn** logistic-regression model over M27 **context_signature** features (one-hot encoded), and emit **`replay_imitation_training_run.json`** + **`replay_imitation_training_run_report.json`** under `out/training_runs/<run_id>/`, with optional **local-only** `joblib` weights referenced by hash (not committed). Bind to **M40** via `training_program_contract_version` + `training_program_contract_sha256`. CLI: `python -m starlab.imitation.emit_replay_imitation_training_run`. Runtime: `docs/runtime/replay_imitation_training_pipeline_v1.md`. Plan: `docs/company_secrets/milestones/M41/M41_plan.md`.

**Local vs CI:** Full training runs are **local-first**. **CI** exercises a **tiny fixture-only** path in the **`tests`** lane (**no** GPU training, **no** live SC2).

**Explicit non-claims:** **not** benchmark-integrity upgrade; **not** ladder / live-play claims; **not** superiority over **M27** beyond bounded recorded metrics; **not** weights in-repo; **not** **M42** comparison harness as in-scope product; **not** closed on `main` until merge + recorded CI — see §1 / §7.

### M40 — Agent Training Program Charter & Artifact Contract — **closed**

**Closed** on `main` ([PR #51](https://github.com/m-cahill/starlab/pull/51); merge commit `44e8edc5bcce8dc99576bf2be542b273095e5072`; merged **2026-04-12T00:52:29Z** (UTC); **final PR head** `be47d913737f322bbf8e9e08a672561c71d322eb`; **authoritative PR-head CI** [`24295050784`](https://github.com/m-cahill/starlab/actions/runs/24295050784); **merge-boundary `main` CI** [`24295326123`](https://github.com/m-cahill/starlab/actions/runs/24295326123); tag **`v0.0.40-m40`** on merge commit `44e8edc…`; branch `m40-agent-training-program-charter` **deleted** after merge; superseded PR-head [`24295030115`](https://github.com/m-cahill/starlab/actions/runs/24295030115) on `6690cd7f0ae79abe0db85695a0d20b4d7c48cdaf` — Ruff format — **not** merge authority; see §18 / `M40_run1.md`). **Proved (narrow, M40):** recharter Phase VI to **M40**–**M45**, **`starlab.training`**, deterministic **`agent_training_program_contract.json`** / **`agent_training_program_contract_report.json`** under `out/training_program/` (via `python -m starlab.training.emit_agent_training_program_contract`), **`docs/runtime/agent_training_program_contract_v1.md`**, ledger **42 → 46** milestones, stub **M41**–**M45** — **not** actual training, **not** weights, **not** benchmark integrity, **not** live SC2 in CI. Plan `docs/company_secrets/milestones/M40/M40_plan.md` (**Complete**); closeout `M40_run1.md`, `M40_summary.md`, `M40_audit.md`.
...
**M40** is **closed** on `main` (see **M40 — closed** above). Until **M41**–**M45** each **merge and close** on `main` with recorded CI, treat the following as **not proved** by default:

- **M41**–**M45** Phase VI **training-track product** milestones (**not** claimed until each closes with recorded CI); **M40** charter artifacts **do not** prove training outcomes or **M41+** product results.
...
**M40 note:** Evidence column reflects **training-program charter** — `starlab.training`, deterministic `agent_training_program_contract.json` / report, runtime contract doc, ledger **46**-milestone arc + **M41**–**M45** stubs — **merged** to `main` (**green PR-head** [`24295050784`](https://github.com/m-cahill/starlab/actions/runs/24295050784) on `be47d91…`; **green merge-push `main`** [`24295326123`](https://github.com/m-cahill/starlab/actions/runs/24295326123) on `44e8edc…`; see §18 / `M40_run1.md`). **Superseded** red PR-head [`24295030115`](https://github.com/m-cahill/starlab/actions/runs/24295030115) (Ruff format — **not** merge authority). **Not** actual training, **not** weights, **not** benchmark integrity, **not** live SC2 in CI, **not** **M41+** product.
