...
**Primary artifacts (intended for M30 closeout):** **`replay_hierarchical_imitation_agent.json`**, **`replay_hierarchical_imitation_agent_report.json`** — deterministic offline hierarchical imitation fit + report (runtime contract `docs/runtime/replay_hierarchical_imitation_agent_v1.md`, product under `starlab/hierarchy/`); **in progress** on branch `m30-first-learned-hierarchical-agent` (see `docs/company_secrets/milestones/M30/M30_plan.md`).
