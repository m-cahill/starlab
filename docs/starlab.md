...
| Current milestone | **M44** — Local Live-Play Validation Harness v1 (**stub**; Phase VI; see §11) — **not** claimed until implemented and closed on `main` with authoritative CI. |
...
| Phase VI local vs CI | Flat training runs **`out/training_runs/`** (M41); hierarchical runs **`out/hierarchical_training_runs/`** (M43); comparison **`out/comparisons/`** (M42). All **local-first**. CI validates **fixture-only** CPU paths (**no** GPU training; **no** live SC2). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M44+** live-play / self-play **product** results (until each milestone closes on `main`), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. |
...
| M44 | Local live-play validation harness v1 (local-only; **stub** until closed on `main`) |
...
| **M44** | **Local live-play validation harness v1** — local-only; **stub** until closed on `main` (see §11). |
...
### M44 — Local Live-Play Validation Harness v1 — **stub** (current)

**Objective:** Local-only live-play validation harness for learned agents — **not** claimed until implemented and closed on `main` with authoritative CI.

**Explicit non-claims:** **not** benchmark integrity; **not** replay↔execution equivalence; **not** live SC2 in CI by default; **not** **M43** hierarchical training scope; **not** **M45** RL / self-play. Plan `docs/company_secrets/milestones/M44/M44_plan.md` (**stub**).
