...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M54** **closed** on `main`; **M55** **active** — benchmark-integrity **charter-only**; **M56**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M54** — Replay↔execution equivalence audit & acceptance gates v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M55** — Benchmark integrity charter & split-governance controls v1 — **active**, **charter-only** (Phase VII; see §11). **M56** — evidence & reproducibility gates — **planned** follow-on. **Not** replay↔execution equivalence (that track is **M52**–**M54**, **closed** on `main`). |
...
| Phase VII pipeline (compact) | **M52**–**M61** — trust, equivalence, benchmark integrity, live SC2-in-CI posture, ladder/public evaluation protocol, audit hardening, v1 release lock — see §6; **M52**–**M54** **closed** on `main` (replay↔execution equivalence: charter → evidence → audit); **M55** **active** — benchmark integrity **charter + split-governance controls** (see **Benchmark integrity track** subsection); **M56**–**M61** **stub/planned** until chartered/closed. |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M55**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
- **M55** — Benchmark integrity charter & split-governance controls v1 (**active** — charter + split-governance controls JSON; contract **`starlab.benchmark_integrity_charter.v1`**; **not** benchmark integrity proved; see **Benchmark integrity track** below).
- **M56** — Benchmark integrity evidence & reproducibility gates v1 (**stub/planned** — evidence/gates; **not** M55).
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M54** **closed** on `main`; **M55** **active** (charter-only; not closed); **M56**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M55 | Benchmark Integrity Charter & Split-Governance Controls v1 | VII | Active | — | — |
...
| M55 | Benchmark integrity charter & split-governance controls v1 — **active** (**charter-only** JSON + runtime contract; **M56** = evidence/gates) |
...
### M55 — Benchmark integrity charter & split-governance controls v1 — **active (charter-only; not closed)**

**Phase VII track separation:** **Replay↔execution equivalence** (**M52**–**M54**, **closed** on `main`) and **benchmark integrity** (**M55**→**M56**) are **adjacent Phase VII tracks** with the same charter→evidence→gates *shape*, but **separate subject matter** — **M55** does **not** collapse into or replace equivalence proofs.

**Status:** current milestone pointer (**Phase VII**). **M55** delivers **charter-only** governance: vocabulary, six **split_governance_controls** (one machine-readable entry per family), and **evidence_classes_reserved_for_m56** — **not** benchmark integrity proof, **not** reproducibility gates, **not** a substitute for **M52**–**M54**.

**Product (in repo):** runtime `docs/runtime/benchmark_integrity_charter_v1.md`; `python -m starlab.benchmark_integrity.emit_benchmark_integrity_charter --output-dir <dir>`; deterministic **`benchmark_integrity_charter.json`** + **`benchmark_integrity_charter_report.json`**; contract id **`starlab.benchmark_integrity_charter.v1`**. **Explicit non-claims:** benchmark integrity **not yet proved**; **no** M56-style evidence/gates in M55; **no** live SC2-in-CI / ladder claims; **no** merge-bar semantics for benchmark integrity. **M56** — evidence & reproducibility gates — **stub/planned**. Folder: `docs/company_secrets/milestones/M55/`.
...
**M51**–**M54** are **closed** on `main`; **M55**–**M61** (**Phase VII**) are **the remaining planned v1 proof track** — **not** implied solved because they appear in the milestone table. **M52** delivered **governance + charter** only; **M53** delivered **evidence surface** JSON/report only (evidence-only posture); **M54** delivered the **profile-scoped audit / acceptance-gate** layer over M53 evidence (bounded, not universal equivalence); **M55** is **active** with **charter-only** benchmark-integrity JSON + runtime contract (**not** closed, **not** integrity proved); **M56**–**M61** remain **stub/planned** until chartered/closed.
...
### 2026-04-15 — **M55** Benchmark integrity charter & split-governance controls v1 — charter product surface (branch / PR)
...
- **Ledger:** §1 status / quick scan; §6 Phase VII **Benchmark integrity track** table + **Phase VII track separation** note; §7 **M55** **Active**; §8 intent map; §11 current milestone narrative; Start Here governance list; §23 (this entry).
