...
| M33 | Audit Closure II — CI Tiering, Architecture Surface, and Field-Test Readiness | V | In progress | v0.0.33-m33 | — |
| M34 | Audit Closure III — Structural Hygiene, Deferred-Issue Closure, and Operating Manual Promotion Prep | V | Planned | v0.0.34-m34 | — |
| M35 | Public Flagship Proof Pack | V | Planned | v0.0.35-m35 | — |
...
**Status:** **Current** milestone — **M33** (**chartered** — `docs/company_secrets/milestones/M33/M33_plan.md`). **Target proof surfaces:** (1) **parallel CI jobs** — `quality`, `smoke`, `tests`, `security`, `fieldtest`, final `governance` (workflow name **`CI`** unchanged); (2) **fixture-only field-test in CI** — `make fieldtest`, assert `replay_explorer_surface.json` + `replay_explorer_surface_report.json`, upload **`out/fieldtest/`** as **`fieldtest-output`**; (3) **docs** — `docs/runtime/ci_tiering_field_test_readiness_v1.md`, `docs/diligence/field_test_session_template.md`, expanded **`docs/architecture.md`** + **`docs/starlab_operating_manual_v0.md`** + clone-to-run / checklist / smoke contract cross-links; (4) **coverage gate** — **75.4** unless variance proven (**DIR-007**). **Not** merged to `main` until PR + authoritative green CI + §18 / §23 closeout sequence.

**M32** is **closed** on `main` ([PR #38](https://github.com/m-cahill/starlab/pull/38); merge commit `cf7219911a208da584537b4c08ab5811fa3f67de`; **authoritative PR-head CI** [`24228528798`](https://github.com/m-cahill/starlab/actions/runs/24228528798); **merge-boundary `main` CI** [`24228788230`](https://github.com/m-cahill/starlab/actions/runs/24228788230); see §18 / `M32_run1.md`); plan `docs/company_secrets/milestones/M32/M32_plan.md` (**Complete**); closeout `M32_run1.md`, `M32_summary.md`, `M32_audit.md`.

**Predecessor (M32):** **Audit Closure I** — measured **`coverage.xml` / `pytest-junit.xml`** in CI, **`fail_under`** line gate, SHA-pinned actions, **`@pytest.mark.smoke`** + `make smoke`, **`Makefile`** (`fieldtest` uses **M31** explorer on fixtures), **`docs/architecture.md`**, **`docs/getting_started_clone_to_run.md`**, **`docs/starlab_operating_manual_v0.md`** (draft), **`docs/audit/DeferredIssuesRegistry.md`**, **38 milestones (M00–M37)** on the ledger; **OD-007** → **M37**.
...
Until **M33** explicitly **merges and closes** on `main` with authoritative CI, treat the following as **not proved**:

- **M33** merge-boundary proof (parallel CI tiers + field-test CI artifact on `main` — **pending** PR).
- **Public flagship proof pack** (**M35** charter).
- **M34** audit-closure III completion.
