...
| Evaluation diagnostics + failure views (M23 consumer) | **Proved (narrow, M24):** deterministic `evaluation_diagnostics.json` + `evaluation_diagnostics_report.json` from one governed **`fixture_only`** **M23** `evaluation_tournament.json`; contract `docs/runtime/evaluation_diagnostics_failure_views_v1.md`, `starlab/evaluation/` — **interpretive**; **no** `starlab.replays`, **no** `starlab.sc2`, **no** `s2protocol` in M24 evaluation modules — **green PR-head** [`24213046380`](https://github.com/m-cahill/starlab/actions/runs/24213046380) on `5caf1fb…`; **green merge-push `main`** [`24213094531`](https://github.com/m-cahill/starlab/actions/runs/24213094531) on `7b4d3b4…`; see §18 / `M24_run1.md`. **Does not** prove evidence packs (**M25**), benchmark integrity, replay↔execution equivalence, or live SC2 in CI. |
| Baseline evidence pack (M21/M22 + M23 + M24 chain) | **In progress (M25):** deterministic `baseline_evidence_pack.json` + `baseline_evidence_pack_report.json` from governed upstream artifacts; contract `docs/runtime/baseline_evidence_pack_v1.md`, `starlab/evaluation/` — **interpretive packaging**; **no** `starlab.replays`, **no** `starlab.sc2`, **no** `s2protocol` in M25 evaluation modules — **not** merged to `main` until M25 closeout records CI + merge commit. **Does not** prove benchmark integrity, new evaluation semantics, replay↔execution equivalence, live SC2 in CI, or **M26** imitation. |
...
| Evaluation diagnostics + failure views (fixture-only) | M24 | **On `main`** — narrow diagnostics + report over governed **M23** tournament JSON (**green PR-head** [`24213046380`](https://github.com/m-cahill/starlab/actions/runs/24213046380) on `5caf1fb…`; **green merge-push `main`** [`24213094531`](https://github.com/m-cahill/starlab/actions/runs/24213094531) on `7b4d3b4…`; see §18 / `M24_run1.md`); **not** evidence packs (**M25**), benchmark integrity, or replay↔execution equivalence |
...
**Status:** **In progress** — **current** milestone; **M24** is **closed** on `main` (see §18 / `M24_run1.md`; `M24_plan.md` **Complete**).

**Goal (high level):** **M25** adds **baseline evidence pack** packaging over the governed **M21/M22 → M23 → M24** chain — deterministic **`baseline_evidence_pack.json`** + **`baseline_evidence_pack_report.json`** — **not** new evaluation semantics and **not** an upgrade to benchmark-integrity claims.

**Primary references:** `docs/company_secrets/milestones/M25/M25_plan.md`; `docs/runtime/baseline_evidence_pack_v1.md`; `docs/starlab.md` §6 Phase IV boundary (M20–M25).

**Note:** **M20–M25** read as **contract/suites → tournament → diagnostics → evidence pack** under explicit **fixture-only** non-claims — **not** benchmark integrity, **not** live SC2 execution in CI.

#### Current milestone — explicit non-claims (standing)

Until **M25** explicitly closes a claim, treat the following as **not proved** for **M25** packaging:

- **Benchmark integrity** / leaderboard claims beyond upstream artifacts (**not** a default proof).
- **New live SC2 execution proof in CI** (CI remains **fixture-driven** unless a milestone explicitly changes that posture).
- **M26** imitation / learning (**stub** until authorized).
