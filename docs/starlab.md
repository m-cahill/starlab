...
| Last closed milestone | **M39** — Public Flagship Proof Pack (merged to `main`; see §11 / §18). |
| Current milestone | **M40** — Agent Training Program Charter & Artifact Contract (Phase VI training track; see §11). |
| Next planned training milestone | **M41** — Replay-Imitation Training Pipeline v1 (**planned**; **not** claimed until closed on `main` with authoritative CI). |
...
- agent training program charter and artifact contract (**M40**)
...
| M40 | Agent Training Program Charter & Artifact Contract | VI | In progress | v0.0.40-m40 | — |
...
| --------- | ------------------------- |
| **M40** | **Agent training program charter & artifact contract** — `starlab.training`, `agent_training_program_contract.json` / report; governance only (**in progress**). |
...
### M40 — Agent Training Program Charter & Artifact Contract

**Status:** **Current** — governance milestone: recharter Phase VI to **M40**–**M45**, emit deterministic **`agent_training_program_contract.json`** / **`agent_training_program_contract_report.json`** under `out/training_program/` (via `python -m starlab.training.emit_agent_training_program_contract`), document **`docs/runtime/agent_training_program_contract_v1.md`**, and seed stub milestones **M41**–**M45**. **M39** is **closed** on `main` (see **M39 — closed** below).

**Explicit non-claims (M40):** **not** actual model training or weights in-repo; **not** GPU training in CI; **not** live SC2 in CI; **not** benchmark integrity or replay↔execution equivalence; **not** ladder or live-play ranking claims; **not** M41–M45 product work until each milestone closes with recorded CI. Training runs remain **local-first**; reference local GPU **RTX 5090 Blackwell** where relevant.
...
Until **M40** is **merged and closed** on `main` with authoritative CI (when implemented), treat the following as **not proved** by default:

- **M41**–**M45** Phase VI **training-track product** milestones (**not** claimed until each closes with recorded CI); **M40** charter artifacts alone do **not** prove training outcomes.
