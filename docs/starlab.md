...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M03`** **closed** on `main` (PR #116–#120). Checkpoint lineage **manifest** `starlab.v15.checkpoint_lineage_manifest.v1` is **metadata** — **not** training **execution**, **not** byte verification by default. Registers + M01–M03 **fixture**-safe contracts; **not** long GPU **training**; **not** claim-critical asset approval. **`V15-M04`** — **not** started. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI** + (full program) data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not satisfied by **PX2** alone | **V15-M04**+ (after explicit approval) |
...
**Current (v1.5):** **`V15-M04`** — *XAI Evidence Contract v1* — **not** started (await **explicit** plan approval). **Authority:** **`docs/starlab-v1.5.md`**. **Prior** M03: **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**.
