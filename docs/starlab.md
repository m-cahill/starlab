...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55** **closed** on `main` — benchmark-integrity **charter-only**; **M56** **active** — bounded evidence/gates v1 — **not** global benchmark integrity proof; **M57**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M55** — Benchmark integrity charter & split-governance controls v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M56** — Benchmark integrity evidence & reproducibility gates v1 — **active** (Phase VII; bounded **fixture-only M21–M25 chain** scope + one gate pack; see §11). **M55** delivered **charter-only** vocabulary + split-governance controls — **closed** on `main`. **M56** adds evidence/gates — **still not** benchmark integrity proved globally. **Not** replay↔execution equivalence (that track is **M52**–**M54**, **closed** on `main`). |
...
| Phase VII pipeline (compact) | **M52**–**M61** — trust, equivalence, benchmark integrity, live SC2-in-CI posture, ladder/public evaluation protocol, audit hardening, v1 release lock — see §6; **M52**–**M54** **closed** on `main` (replay↔execution equivalence: charter → evidence → audit); **M55** **closed** on `main` — benchmark integrity **charter + split-governance controls**; **M56** **active** — bounded benchmark-integrity **evidence + reproducibility gates** (fixture-only baseline chain — **not** global proof); **M57**–**M61** **stub/planned** until chartered/closed. |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M56**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
- **M56** — Benchmark integrity evidence & reproducibility gates v1 (**active** — runtime `docs/runtime/benchmark_integrity_evidence_reproducibility_gates_v1.md`; `starlab.benchmark_integrity` deterministic `benchmark_integrity_evidence.json` / report + `benchmark_integrity_reproducibility_gates.json` / report; scope **`starlab.m56.scope.fixture_only_baseline_chain_v1`**; gate pack **`starlab.m56.gatepack.fixture_only_baseline_chain_reproducibility_v1`**; explicit path CLIs — **not** global benchmark integrity proof).
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M55** **closed** on `main` (through benchmark-integrity **charter-only** milestone); **M56**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M56 | Benchmark Integrity Evidence & Reproducibility Gates v1 | VII | Active | — | — |
...
### M56 — Benchmark integrity evidence & reproducibility gates v1 — **active (implementation)**
...
**Status:** current milestone pointer (**Phase VII**). **Depends on:** **M55** **closed** on `main` — charter-only JSON — **not** benchmark integrity proved.
...
### 2026-04-15 — **M56** Benchmark integrity evidence & reproducibility gates v1 — implementation (branch)

- **Product (narrow, bounded):** runtime `docs/runtime/benchmark_integrity_evidence_reproducibility_gates_v1.md`; `starlab.benchmark_integrity` deterministic **`benchmark_integrity_evidence.json`** / **`benchmark_integrity_evidence_report.json`** (contract **`starlab.benchmark_integrity_evidence.v1`**) + **`benchmark_integrity_reproducibility_gates.json`** / **`benchmark_integrity_reproducibility_gates_report.json`** (contract **`starlab.benchmark_integrity_reproducibility_gates.v1`**); CLIs `python -m starlab.benchmark_integrity.emit_benchmark_integrity_evidence` and `python -m starlab.benchmark_integrity.emit_benchmark_integrity_gates`; scope **`starlab.m56.scope.fixture_only_baseline_chain_v1`** (explicit M21–M25 paths only); gate pack **`starlab.m56.gatepack.fixture_only_baseline_chain_reproducibility_v1`**; fixture tests `tests/test_m56_benchmark_integrity_evidence_reproducibility_gates.py` — **not** global benchmark integrity proof; **not** M52–M54 equivalence claims; **not** live SC2 in CI / ladder claims.
- **Ledger:** §1 status / quick scan / §6 Phase VII + bounded benchmark-integrity scope/gatepack tables / §7 **M56** **Active** / §11 **M56** section; **PR** and **merge** pending — this ledger entry documents the **implementation surface** on the feature branch.
