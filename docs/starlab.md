...
| **Later `PX2-M03` execution** | Operator-local industrial self-play (Blackwell-class **intent**), long-run checkpoints/evals/promotion as implemented — **not** claimed by slices 1–8 |
...
| `PX2-M03` | Industrial Self-Play Campaign | **open** on `main` | Preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**; **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** — slices 1–8 (through bounded operator-local multi-run session). Industrial run — **later** in milestone — **not** **PX2-M04**; **not** demo refresh |
...
- **`PX2-M03`** **open** on `main` — industrial self-play **campaign** milestone (operator-local / Blackwell-class intent) — **readiness** **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`** — **slice 1** + **slice 2** + **slice 3** + **slice 4** + **slice 5** + **slice 6** + **slice 7** + **slice 8** (preflight + bounded operator-local smoke + multi-step continuity + promotion/rollback receipts + campaign-root manifest + opponent rotation + preflight seal normalization + canonical operator-local smoke path + bounded real-run execution record + bounded multi-run session) on `main` — **not** industrial Blackwell-scale completion; **not** merge-gate long-run execution; **not** **PX2-M04** promotion/exploit closure.
...
| PX2-M03 | Industrial self-play **campaign** (operator-local / Blackwell-class intent; checkpoints/evals/promotion/rollback/opponent pool; slices 1–8 through bounded multi-run session — **open** on `main` — preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**) — **not** M04 promotion closure; **not** campaign **evidence** by opening alone |
...
**Non-claims:** **not** self-play **evidence** in-repo; **not** Blackwell run **completed** by opening; **not** strength proof; slices 1–8 **not** substitute for industrial **`PX2-M03`** execution phase.
