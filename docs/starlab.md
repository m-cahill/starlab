...
| ----- | ------------------------ |
| Program arc | **47 milestones (M00–M46)** — see §7. |
| Last closed milestone | **M45** — Self-Play / RL Bootstrap v1 (merged to `main`; see §11 / §18). **M46** — bounded live validation final-status semantics — **pending merge** (feature branch; see §1 lead-in). |
| Current milestone | **M46** (branch) — aligns **M44** burnysc2 bounded `final_status` with fixture validation semantics; **M00–M45** arc **complete** on `main`. |
...
| Phase VI pipeline (compact) | **M40** contract → **M41** flat training run → **M42** comparison → **M43** hierarchical training → **M44** local live-play → **M45** self-play / RL → **M46** bounded live `final_status` / `sc2_game_result` semantics (merge pending) |
...
| M46 | Bounded Live Validation Final-Status Semantics | VI | Complete (branch; pending merge) | v0.0.46-m46 | — |
...
#### Post–M45 closure — explicit non-claims (standing)

**M40**–**M45** are **closed** on `main` with recorded CI (see §18). Treat the following as **not proved** by default:
...
### 2026-04-13 — **M46** bounded live validation final-status semantics (Option A; branch `recharter/m44-bounded-live-final-status-semantics`)

- **Governance input:** `docs/diligence/m44_bounded_live_final_status_recharter.md` — **Option A:** when bounded **burnysc2** runs record `bounded_exit` in the execution proof `status_sequence`, **`match_execution.final_status`** is **`ok`** (governed validation-contract completion at step cap); literal SC2 `Result` is preserved as **`sc2_game_result`** on `match_execution_proof.json` and (when present) on **`match_execution`** in `local_live_play_validation_run.json`. **Not** match victory, **not** ladder strength, **not** benchmark integrity, **not** replay↔execution equivalence, **not** live SC2 in CI.
- **Code / tests:** `starlab/sc2/adapters/burnysc2_adapter.py`, `starlab/sc2/artifacts.py`, `starlab/sc2/local_live_play_validation_harness.py`; fixture tests `tests/test_m46_bounded_live_validation_semantics.py`, `tests/test_sc2_artifacts.py`; runtime docs `docs/runtime/local_live_play_validation_harness_v1.md`, `docs/runtime/self_play_rl_bootstrap_v1.md`; operator campaign framing `docs/diligence/phase_vi_integrated_test_campaign.md`; recharter `docs/diligence/m44_bounded_live_final_status_recharter.md`; milestone package `docs/company_secrets/milestones/M46/`.
- **Program arc:** ledger updated to **47 milestones (M00–M46)** — §6–§7.
- **PR / CI:** [PR #57](https://github.com/m-cahill/starlab/pull/57) — **authoritative green PR-head CI** [`24332502917`](https://github.com/m-cahill/starlab/actions/runs/24332502917) (**success**; required jobs: **`quality`**, **`smoke`**, **`tests`**, **`security`**, **`fieldtest`**, **`flagship`**, **`governance`**); superseded earlier greens on same PR [`24332444125`](https://github.com/m-cahill/starlab/actions/runs/24332444125), [`24332375680`](https://github.com/m-cahill/starlab/actions/runs/24332375680), [`24332314901`](https://github.com/m-cahill/starlab/actions/runs/24332314901). **Merge-boundary `main` CI** — **TBD** at merge time. **Out of scope:** **M42** `--contract` path mismatch — separate follow-on.
