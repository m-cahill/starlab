...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M60** **closed** on `main`; **M61** **machinery merged**; **operator-local release-lock evidence** **pending**). |
| Last closed milestone | **M60** — Audit hardening & v2 readiness v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M61** — SC2 foundation release lock & v1 proof pack — **machinery merged on `main`** ([PR #72](https://github.com/m-cahill/starlab/pull/72); tag **`v0.0.61-m61`**); **operator-local** campaign + proof pack + audit with **`ready_within_scope`** **still pending** — see §11. **M60** **closed** — structural/diligence (private campaign executor split + guardrails + audit mapping) — **not** new governed product artifact family; **not** equivalence/benchmark/ladder/live-SC2 claims. **M59** **closed** — bounded descriptive ladder/public protocol + evidence — **not** ladder/public **performance** proof; **not** benchmark integrity; **not** replay↔execution equivalence (**M52**–**M54** **closed**). **M58** **closed** — bounded **M57** hardening + cost guardrails — **not** global live-SC2-in-CI proof. |
| Next planned follow-on | **Phase VI integrated test campaign** as **mandatory M61 release-lock evidence input** (operator-local industrial hidden rollout path; **not** default CI) — see `docs/diligence/phase_vi_integrated_test_campaign.md` — still **not** claimed complete unless separately executed and evidenced into an M61 proof pack + audit. |
...
| Phase VII pipeline (compact) | **M52**–**M61** — trust, equivalence, benchmark integrity, live SC2-in-CI posture, ladder/public evaluation protocol, audit hardening, v1 release lock — see §6; **M52**–**M54** **closed** on `main` (replay↔execution equivalence: charter → evidence → audit); **M55**–**M56** **closed** on `main` — benchmark integrity **charter + split-governance controls** (**M55**) + bounded **evidence + reproducibility gates** (**M56** — **not** global proof); **M57** narrow live-SC2-in-CI **charter + controlled runner** (**closed** on `main`); **M58** **hardening + cost guardrails v1** (**closed** on `main` — preflight + policy artifacts — **not** merge-gate live SC2); **M59** **ladder/public evaluation protocol + evidence** (**closed** on `main` — descriptive packaging — **not** ladder strength); **M60** **audit hardening & v2 readiness** (**closed** on `main` — structural/diligence); **M61** **machinery merged** (release-lock emitters + docs/tests on `main`; **operator-local** campaign evidence + **`ready_within_scope`** **pending**). |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI as default merge-gate / global proof**, **ladder / public performance**, **Phase VI integrated test campaign** **as fully evidenced M61 release-lock closure** (operator-local packaging TBD until executed), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. The **remaining v1 foundation-completion arc** is **M61** only — **machinery is on `main`**; **within-scope release-lock closure** (`ready_within_scope`) is **still pending** operator-local evidence — not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
**Present tense (honest):** **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI as merge-gate proof**, and **ladder/public performance** are **not** proved **yet**. They are **planned within the remaining v1 arc** (**M52**–**M61**), chartered milestone-by-milestone — **not** silently relegated to “after v1.”

| **v1 means (today + planned remaining arc)** | **v1 does *not* mean yet (still not proved by default)** |
| ----- | ----- |
...
#### Remaining v1 proof-track map (planned; **not** merge-gated as complete)

Compact roadmap for **Phase VII** — **stub/planned** rows in §7 until each milestone closes; **no** implication that any row is already satisfied.
...
| M61 | `starlab.sc2_foundation_v1_proof_pack.v1`; `starlab.sc2_foundation_release_lock_audit.v1` | `sc2_foundation_v1_proof_pack.json`; `sc2_foundation_release_lock_audit.json` (+ reports) | Within-scope release lock packaging over prior milestone anchors + one operator-local campaign profile — **not** benchmark universality; **not** replay↔execution universality; **not** ladder strength; **`ready_within_scope` is blocked** until post-bootstrap + watchable M44 + operator-declared full-run threshold are evidenced |
...
**M61 operator-local closure** (`release_scope_status`: `ready_within_scope`) remains **pending** after **machinery merge** on `main` — an operator-designated campaign must still be executed and captured **without** committing raw `out/` secrets to the repo.
...
- **M61** — SC2 foundation release lock & v1 proof pack (**machinery merged on `main`** — proof pack + audit machinery in repo; **operator-local** release evidence + **`ready_within_scope`** **pending**).

---

## 7. Milestone table

Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M60** **closed** on `main` (through benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) + narrow live-SC2-in-CI **charter + controlled runner** (**M57**) + **M58** guardrails/hardening + **M59** ladder/public descriptive protocol/evidence + **M60** audit hardening milestones); tag **`v0.0.60-m60`** on **M60** merge commit; **M61** **machinery merged** (tag **`v0.0.61-m61`** on PR #72 merge commit); **operator-local release-lock evidence** **pending**; **M51** **closed** on `main`):
...
| M61 | SC2 Foundation Release Lock & v1 Proof Pack | VII | Machinery merged; evidence pending | v0.0.61-m61 | — |
...
| M61 | SC2 foundation release lock & v1 proof pack — **machinery merged on `main`** (release-lock emitters + docs; **operator-local** release-lock closure **`ready_within_scope`** **pending**) |
...
### M61 — SC2 foundation release lock & v1 proof pack — **in progress**

**Machinery merged on `main`** ([PR #72](https://github.com/m-cahill/starlab/pull/72); merge commit `35d7734d14113adf206390f153f517a93d7d41ba` merged **2026-04-16T05:33:27Z** (UTC); **authoritative PR-head CI** [`24493016581`](https://github.com/m-cahill/starlab/actions/runs/24493016581) — **success**; **merge-boundary `main` CI** [`24493963087`](https://github.com/m-cahill/starlab/actions/runs/24493963087) on merge commit `35d7734…` — **success**; tag **`v0.0.61-m61`** on merge commit **`35d7734…`**). Branch `m61-sc2-foundation-release-lock-v1` **retained** on `origin` as of this alignment pass. **Operator-local** campaign evidence + proof pack + audit with **`release_scope_status`: `ready_within_scope`** remain **pending** — **not** claimed from default CI.

**Phase VII** — **v1 foundation-completion** milestone after **M60**. **Product (in-repo machinery):** runtime `docs/runtime/sc2_foundation_release_lock_v1.md`; deterministic **`sc2_foundation_v1_proof_pack.json`** / **`sc2_foundation_v1_proof_pack_report.json`** (`starlab.sc2_foundation_v1_proof_pack.v1`); **`sc2_foundation_release_lock_audit.json`** / **`sc2_foundation_release_lock_audit_report.json`** (`starlab.sc2_foundation_release_lock_audit.v1`); emitters `python -m starlab.release_lock.emit_sc2_foundation_v1_proof_pack` and `python -m starlab.release_lock.emit_sc2_foundation_release_lock_audit`; operator-authored **`proof_pack_input.json`** contract (`starlab.sc2_foundation_v1_proof_pack_input.v1`); **`tests/test_m61_sc2_foundation_release_lock.py`** + `tests/fixtures/m61/` — **fixture-only** M44/campaign rows — **`release_scope_status`: `ready_within_scope`** is **blocked** unless the proof pack records **post-bootstrap protocol phases**, **`watchable_m44_validation` executed**, **`operator_declared_full_run` + `threshold_satisfied`**, and explicit **non-claims** (see audit checks).

**Operator-local closure (not default CI):** one **fresh M61-designated** operator-local **`python -m starlab.training.execute_full_local_training_campaign`** run with **`--post-bootstrap-protocol-phases`**, governed **M49** contract + preflight, then emit proof pack + audit from resulting artifacts — raw outputs remain under **`out/`** locally; **do not** commit `docs/company_secrets/` or heavy `out/` trees.
...
| v1 release lock / proof pack | — | — | Yes | **M61** |
...
**M51**–**M60** are **closed** on `main`; **M61** (**Phase VII** continuation) is **the remaining v1 proof-track milestone** — **not** implied solved because it appears in the milestone table. **M52** delivered **governance + charter** only; **M53** delivered **evidence surface** JSON/report only (evidence-only posture); **M54** delivered the **profile-scoped audit / acceptance-gate** layer over M53 evidence (bounded, not universal equivalence); **M55** delivered **charter-only** benchmark-integrity JSON + runtime contract (**closed** on `main` — **not** integrity proved); **M56**–**M60** are **closed** on `main` (**M60** structural/diligence only); **M61** **machinery is merged on `main`** (proof pack + audit tooling); **within-scope release-lock closure** (`ready_within_scope`) remains **pending** until **fresh** operator-local campaign evidence is packaged — **not** `ready_within_scope` from repo CI alone.
...
- **Closeout:** `M60_run1.md`, `M60_summary.md`, `M60_audit.md` (**local only** under `docs/company_secrets/` — **not** committed); §1 / quick scan / §6 / §7 **M60** **Complete** + tag **`v0.0.60-m60`** / §11 current milestone pointer **M61** (**machinery merged**; **operator-local evidence pending**) + **M60** **closed** section / §18 / §23; `docs/runtime/v2_readiness_audit_hardening_v1.md` header (**M60** **closed** on `main`); governance tests (`tests/test_m58_live_sc2_in_ci_hardening_cost_guardrails.py`) ledger pointer checks.
