...
| `PX2-M03` | Industrial Self-Play Campaign | **open** on `main` | Preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**; **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** — slices 1–16 (through handoff-anchored bounded run) + **§8q bounded substantive** (`px2_self_play_bounded_substantive_execution.json`). Industrial run — **later** in milestone — **not** **PX2-M04**; **not** demo refresh |
...
- **`PX2-M03`** **open** on `main` — industrial self-play **campaign** milestone (operator-local / Blackwell-class intent) — **readiness** **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`** — **slice 1** + **slice 2** + **slice 3** + **slice 4** + **slice 5** + **slice 6** + **slice 7** + **slice 8** + **slice 9** + **slice 10** + **slice 11** + **slice 12** + **slice 13** + **slice 14** + **slice 15** + **slice 16** + **§8q bounded substantive** (`px2_self_play_bounded_substantive_execution.json`; after slice 16, PX2-M03 exits lineage-surface expansion and enters bounded substantive execution, still below industrial scale) (preflight + bounded operator-local smoke + multi-step continuity + promotion/rollback receipts + campaign-root manifest + opponent rotation + preflight seal normalization + canonical operator-local smoke path + bounded real-run execution record + bounded multi-run session + bounded session transition + current-candidate carry-forward + continuation run consuming current-candidate + post-continuation current-candidate re-anchoring + second-hop continuation / optional symmetric re-anchor + pointer-seeded bounded run + post–pointer-seeded handoff + handoff-anchored bounded run + bounded substantive post–slice-16 execution) on `main` — **not** industrial Blackwell-scale completion; **not** merge-gate long-run execution; **not** **PX2-M04** promotion/exploit closure.
...
| PX2-M03 | Industrial self-play **campaign** (operator-local / Blackwell-class intent; checkpoints/evals/promotion/rollback/opponent pool; slices 1–16 through handoff-anchored bounded run + **§8q bounded substantive** — **open** on `main` — preflight **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**) — **not** M04 promotion closure; **not** campaign **evidence** by opening alone |
