...
| M28 | Learned-Agent Evaluation Harness | V | Complete (merge pending) | v0.0.28-m28 | — |
...
| **M28** | **Learned-agent evaluation harness** — deterministic `learned_agent_evaluation.json` / `learned_agent_evaluation_report.json` (M20 contract + frozen **M27** + **M26** `test` + **M14**); **product in-repo**; merge + CI **pending** (see §11). |
| **M29** | **Hierarchical agent interface layer** (planned; **stub-only**). |
...
### M29 — Hierarchical Agent Interface Layer (stub)

**Status:** **Current** milestone — **M28** product scope is **implemented** in-repo (`starlab/evaluation/` learned-agent evaluation harness + `starlab/imitation/replay_imitation_predictor.py`); **merge to `main` + authoritative CI** are the merge gate (record in §18 when closed). **M29** is **stub-only** for product code — see `docs/company_secrets/milestones/M29/M29_plan.md`.

**Predecessor (M28):** **Learned-agent evaluation harness** — deterministic offline **`learned_agent_evaluation.json`** / **`learned_agent_evaluation_report.json`** from: one governed **M20** `fixture_only` benchmark contract (v1 metric set: `accuracy`, `macro_f1`, `fallback_rate`, `example_count`), frozen **`replay_imitation_baseline.json`**, governed **`replay_training_dataset.json`**, referenced **M14** bundle directories; embedded **M20-compatible** scorecard; runtime contract `docs/runtime/learned_agent_evaluation_harness_v1.md`; CLI `python -m starlab.evaluation.emit_learned_agent_evaluation`. **Explicit non-claims:** not benchmark integrity, not live SC2, not M23–M25 surfaces.

#### Current milestone — explicit non-claims (standing)

Until **M29** explicitly closes its chartered scope on `main`, treat the following as **not proved**:
...
- **Flagship hierarchical proof** beyond the **M29** charter when authorized.
