...
| Current milestone | **`V15-M04`** — *XAI Evidence Contract v1* — **in progress** (contract `starlab.v15.xai_evidence_pack.v1`; runtime **`docs/runtime/v15_xai_evidence_contract_v1.md`**). **Last closed (v1.5):** **`V15-M03`** on `main` ([PR #120](https://github.com/m-cahill/starlab/pull/120); merge `47a3fcb0…`; PR-head [`24919300604`](https://github.com/m-cahill/starlab/actions/runs/24919300604); merge-boundary [`24920070670`](https://github.com/m-cahill/starlab/actions/runs/24920070670)). **Authority** **`docs/starlab-v1.5.md`**; M03 runtime (closed) **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02 runtime **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** perform a **long wall-clock GPU training session** as a program deliverable. Default **CI** remains **CPU** fixture self-play / bootstrap paths. **`PX2-M04`** / **`PX2-M05`** — **planned**; **`PX1-M05`** optional; **v2** not opened. |
...
| Next planned follow-on | **`V15-M04`** (XAI evidence contract) — **in progress** (see **`docs/runtime/v15_xai_evidence_contract_v1.md`**). **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only** on the **PX2** namespace. **`PX1-M05`** optional. **`docs/starlab-vision.md`** **v2** directions remain **outside** the closed **M00–M61** arc until **explicit** recharter — **v2** is **deferred** / **not** opened. |
...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M03`** **closed** on `main` (PR #116–#120). Checkpoint lineage **manifest** `starlab.v15.checkpoint_lineage_manifest.v1` is **metadata** — **not** training **execution**, **not** byte verification by default. **`V15-M04`** XAI evidence contract **`starlab.v15.xai_evidence_pack.v1`** — **in progress** (fixture + schema; **not** real XAI inference). Registers + M01–M04 **fixture**-safe contracts; **not** long GPU **training**; **not** claim-critical asset approval. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI demo** + (full program) data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not satisfied by **PX2** alone | **V15-M05**+ after M04 merge |
