...
| M23 | `evaluation_tournament.json`, `evaluation_tournament_report.json` | M20 **`fixture_only`** benchmark contract + one or more governed **M21/M22** suite artifacts; deterministic entrant catalog + round-robin harness over embedded scorecards | STARLAB JSON; **no** `starlab.replays`, **no** `starlab.sc2`, **no** `s2protocol` in M23 `starlab/evaluation/` modules | Diagnostics views (**M24**), evidence packs (**M25**), benchmark integrity, replay↔execution equivalence, live SC2 in CI |
| M24 | `evaluation_diagnostics.json`, `evaluation_diagnostics_report.json` | One governed **M23** `evaluation_tournament.json` (`fixture_only`); deterministic entrant/match/standing explanations + failure-view surfaces (**interpretive** — does not re-score) | STARLAB JSON; **no** `starlab.replays`, **no** `starlab.sc2`, **no** `s2protocol` in M24 `starlab/evaluation/` modules | Evidence packs (**M25**), benchmark integrity, replay↔execution equivalence, live SC2 in CI |

**Phase IV boundary (M20–M24):** **M20** defines benchmark contract + scorecard schemas. **M21** and **M22** are **fixture-only baseline suite emitters** (scripted vs heuristic subjects). **M23** consumes those suites and proves a **fixture-only evaluation runner + tournament harness** (deterministic tournament artifacts). **M24** consumes **M23** output and emits **diagnostic / failure-view** JSON — **not** benchmark integrity, **not** live SC2 execution, **not** replay-derived scoring, **not** evidence packaging (**M25**).
...
| Evaluation diagnostics + failure views (fixture-only) | M24 | **At M24 closeout** — narrow diagnostics + report over governed **M23** tournament JSON; **authoritative PR-head + merge-boundary CI** recorded in §18; **not** evidence packs (**M25**), benchmark integrity, or replay↔execution equivalence |
...
| **Runner / harness non-claims** | **Not** benchmark integrity, leaderboard validity, replay↔execution equivalence, live SC2, replay parsing, attribution/diagnostics (**M24**), or evidence packs (**M25**). |
...
| **M24** | **Evaluation diagnostics + failure views** — load one governed **M23** `evaluation_tournament.json`; deterministic `evaluation_diagnostics.json` / `evaluation_diagnostics_report.json` (**interpretive**; **not** new scoring semantics). |
...
**Status:** **Planned** — **current** milestone; **M24** is **closed** at the product/ledger level (see `docs/company_secrets/milestones/M24/M24_plan.md`; authoritative **PR/CI** for M24 at **closeout** in §18).

**Goal (high level):** **M25** introduces **baseline evidence pack** packaging — **not** part of **M24** diagnostics interpretation.

**Primary references:** `docs/company_secrets/milestones/M25/M25_plan.md` (stub); `docs/starlab.md` §6 Phase IV boundary (M20–M25).

**Note:** **M20–M24** establish **benchmark contract → baseline suite emitters → evaluation tournament → diagnostics views** under explicit **fixture-only** non-claims — **not** benchmark integrity, **not** live SC2 execution in CI.

#### Current milestone — explicit non-claims (standing)

Until **M25** explicitly closes a claim, treat the following as **not proved** for **M25** planning:
...
- **Baseline evidence pack** (**M25** — stub product code until authorized).
...
**M23 note:** Evidence column reflects **fixture-only evaluation tournament** (`evaluation_tournament.json` / `evaluation_tournament_report.json`) from M20 + **M21/M22** suite artifacts + full governance CI **on `main`** (**green PR-head** [`24178571859`](https://github.com/m-cahill/starlab/actions/runs/24178571859) on `f00711a…`; **green merge-push `main`** [`24178615940`](https://github.com/m-cahill/starlab/actions/runs/24178615940) on `b8857d2…`; see §18 / `M23_run1.md`). **Not** evaluation diagnostics (**M24**), evidence packs (**M25**), benchmark integrity, replay↔execution equivalence, or live SC2 in CI.

**M24 note:** Evidence column reflects **fixture-only evaluation diagnostics** (`evaluation_diagnostics.json` / `evaluation_diagnostics_report.json`) from one governed **M23** `evaluation_tournament.json` + full governance CI — **authoritative PR-head + merge-boundary `main` CI at M24 closeout** (§18). **Interpretive** over M23 — **not** new tournament semantics. **Not** evidence packs (**M25**), benchmark integrity, replay↔execution equivalence, or live SC2 in CI.
...
### 2026-04-09 — M24 evaluation diagnostics (implementation)

- Product: `docs/runtime/evaluation_diagnostics_failure_views_v1.md`, `starlab/evaluation/diagnostics_models.py`, `diagnostics_views.py`, `emit_evaluation_diagnostics.py`, `tests/fixtures/m24/`, `tests/test_evaluation_diagnostics.py` (includes **M20 → M21/M22 emitters → M23 → M24** chain test); **M25** stubs only (`docs/company_secrets/milestones/M25/`)
- §3 / §6 / §7 / §10 / §11 / §18 / §20 / §23: Phase IV chain extends through **M24** diagnostics; **current milestone** → **M25** (planned); authoritative **PR/CI** for M24 at **merge closeout**
