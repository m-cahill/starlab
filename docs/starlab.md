...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M56** **closed** on `main` — benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) — **not** global benchmark integrity proof; **M57** **active** (narrow live-SC2-in-CI charter + controlled runner — **not** merge-gate proof); **M58**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M56** — Benchmark integrity evidence & reproducibility gates v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M57** — Narrow live SC2 in CI charter & controlled runner v1 — **in progress** (Phase VII; charter + controlled-runner receipts over **M44**; **not** default merge-gate live SC2). **M56** **closed** — bounded benchmark-integrity evidence + gates — **not** global proof. **M58** next (hardening + cost guardrails). **Not** replay↔execution equivalence (**M52**–**M54** **closed**). |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M57**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
| --- | --- | --- | --- |
| `starlab.m57.runner_profile.m44_single_validation_v1` | One M44 validation run; one M43 candidate; one output dir; one replay-binding chain | `run_local_live_play_validation` (**M44**) | Global live SC2 in CI **not** proved as merge norm; **M58** cost/hardening **out of scope** |
...
- **M57** — Narrow live SC2 in CI charter & controlled runner v1 (**in progress** — runtime + `starlab.sc2` charter/receipt emitters + optional manual workflow; **not** merge-gate live SC2).
- **M58** — Live SC2 in CI hardening & cost guardrails v1 (**stub/planned** — follows **M57**).
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M56** **closed** on `main` (through benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) milestones); **M57**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M57 | Narrow Live SC2 in CI Charter & Controlled Runner v1 | VII | In progress | — | — |
...
| M57 | Narrow live SC2 in CI charter & controlled runner v1 — **in progress** (charter + controlled runner over **M44**; **not** merge-gate live SC2) |
...
### M57 — Narrow live SC2 in CI charter & controlled runner v1 — **in progress**

**Status:** current milestone (**Phase VII**). **Depends on:** **M56** **closed** on `main` — bounded benchmark-integrity evidence + reproducibility gates — **not** global benchmark integrity proved.
...
**Explicit non-claims:** **no** default merge-gate live SC2; **no** silent downgrade from `local_live_sc2` to `fixture_stub_ci`; **no** **M58** hardening/cost controls here; **no** ladder/public evaluation; benchmark integrity / replay↔execution equivalence remain **separate** tracks. **Next stub:** **M58** — Live SC2 in CI hardening & cost guardrails v1. Folder: `docs/company_secrets/milestones/M57/`.
...
**M51**–**M55** are **closed** on `main`; **M56**–**M61** (**Phase VII** continuation) are **the remaining planned v1 proof track** — **not** implied solved because they appear in the milestone table. **M52** delivered **governance + charter** only; **M53** delivered **evidence surface** JSON/report only (evidence-only posture); **M54** delivered the **profile-scoped audit / acceptance-gate** layer over M53 evidence (bounded, not universal equivalence); **M55** delivered **charter-only** benchmark-integrity JSON + runtime contract (**closed** on `main` — **not** integrity proved); **M56**–**M61** remain **stub/planned** until chartered/closed.
...
### 2026-04-15 — **M57** Narrow live SC2 in CI charter & controlled runner v1 — implementation in repo (pending merge)

- **Governance:** runtime `docs/runtime/live_sc2_in_ci_charter_controlled_runner_v1.md`; `starlab.sc2` deterministic **`live_sc2_in_ci_charter.json`** / **`live_sc2_in_ci_charter_report.json`** (contract **`starlab.live_sc2_in_ci_charter.v1`**); **`live_sc2_in_ci_controlled_runner_receipt.json`** / **`live_sc2_in_ci_controlled_runner_receipt_report.json`** (contract **`starlab.live_sc2_in_ci_controlled_runner_receipt.v1`**) wrapping **`run_local_live_play_validation`**; profile **`starlab.m57.runner_profile.m44_single_validation_v1`**; optional **`.github/workflows/live-sc2-controlled-runner.yml`** (`workflow_dispatch` only).
- **Explicit non-claims:** **not** default merge-gate live SC2; **not** **M58** hardening/cost guardrails; **not** benchmark-integrity / replay↔execution equivalence expansion.
- **Ledger:** §1 / quick scan / Phase VII bounded live-SC2 runner profile table / §6 / §7 **M57** **In progress** / §11 / §23.
