...
| **M27** | **Replay-derived imitation baseline (narrow, trained artifact)** — deterministic `replay_imitation_baseline.json` / `replay_imitation_baseline_report.json` from governed **M26** + **M14** (majority-label-per-context-signature; explicit non-claims; **not** benchmark integrity, **not** **M28** harness) (**proved** on `main`; see §18 / [PR #33](https://github.com/m-cahill/starlab/pull/33)). |
...
### M28 — Learned-Agent Evaluation Harness (stub)

**Status:** **Current** milestone — **M27** is **closed** on `main` (see §18 / `M27_run1.md`; `M27_plan.md` **Complete**). **M27** proved the **first deterministic, offline, replay-derived trained imitation baseline artifact** (`replay_imitation_baseline.json` / `replay_imitation_baseline_report.json`) over governed **M26** + **M14** with explicit non-claims ([PR #33](https://github.com/m-cahill/starlab/pull/33)). **M28** is **stub-only** until chartered — **no** M28 product code on `main` at M27 closeout.

**Goal (when authorized):** Prove a **learned-agent evaluation harness** with bounded semantics and explicit non-claims (building on **M20–M25** patterns where applicable). **Not** benchmark integrity by default, **not** live SC2 in CI by default.

**Primary references:** `docs/company_secrets/milestones/M28/M28_plan.md` (stub); `docs/starlab.md` §6 Phase V boundary.

**Standing context:** **M27** used **in-process M16 → M18** materialization only; **`agreement_by_split`** metrics are **internal smoke only** — not benchmark claims.

#### Current milestone — explicit non-claims (standing)

Until **M28** explicitly closes its chartered scope on `main`, treat the following as **not proved**:
...
- **Hierarchical agents** / flagship proof surfaces reserved for later Phase V/VI milestones.
