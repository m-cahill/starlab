...
| Current milestone | **`V15-M07`** — *Training Smoke and Short GPU Shakedown* — **in progress** (governed **`starlab.v15.training_run_receipt.v1`**, `python -m starlab.v15.emit_v15_training_run_receipt`, **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**; bounded shakedown only; **not** **V15-M08** long campaign; **`long_gpu_run_authorized` false**). **Last closed (v1.5):** **`V15-M06`** on `main` ([PR #127](https://github.com/m-cahill/starlab/pull/127); **not** human-panel **execution**; **not** “beats most humans” **claim**; protocol + `fixture_ci` + optional **operator_declared**). M06 (closed) **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**; M05 **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**. M04 **`docs/runtime/v15_xai_evidence_contract_v1.md`**. M03 **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**. M02 **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**. Default **CI** remains **CPU** fixture self-play / bootstrap. **`PX2-M04`** / **`PX2-M05`** — **planned**; **v2** not opened. |
...
| Next planned follow-on | After **M07** closeout: **`V15-M08`** — *Long GPU Campaign Execution* — **not** started until explicit approval (see **`docs/starlab-v1.5.md`**). **M07** = training-run receipt + optional operator shakedown — **not** M08. **M06 (closed):** human-panel **protocol** + **fixture** only. **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only**. **v2** **deferred**. |
...
| **v1.5 (V15)** | **`V15-M00`** through **`V15-M06`** **closed** on `main` ([PR #127](https://github.com/m-cahill/starlab/pull/127) for M06); **`V15-M07`** — *Training Smoke and Short GPU Shakedown* — **in progress** (`starlab.v15.training_run_receipt.v1`, `docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`; **not** M08). M06: protocol + `fixture_ci` + optional **operator_declared** — **not** human-panel **execution**; see **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**. | Full GPU run + **strong-agent** eval + XAI + **human-panel** eval — **chartered in v1.5**, not satisfied by **PX2** alone | **M08+** (long run), then eval / human / XAI per roadmap |
...
**Current (v1.5):** **`V15-M07`** — *Training Smoke and Short GPU Shakedown* — **in progress** (governed `starlab.v15.training_run_receipt.v1` + `emit_v15_training_run_receipt`; **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**; **not** **V15-M08**). **Closed v1.5 line on `main`:** **`V15-M00`**, **`V15-M01`**, **`V15-M02`**, **`V15-M03`**, **`V15-M04`**, **`V15-M05`**, **`V15-M06`**. **Authority:** **`docs/starlab-v1.5.md`**. M07 runtime: **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**; M06 (closed): **`docs/runtime/v15_human_panel_benchmark_protocol_v1.md`**; M05: **`docs/runtime/v15_strong_agent_benchmark_protocol_v1.md`**; M04: **`docs/runtime/v15_xai_evidence_contract_v1.md`**; M03: **`docs/runtime/v15_checkpoint_lineage_resume_discipline_v1.md`**; M02: **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**; M01: **`docs/runtime/v15_training_scale_provenance_asset_registers_v1.md`**.

### V15-M07 — *Training Smoke and Short GPU Shakedown* — **in progress**

V15-M07 closed — *Training smoke and short GPU shakedown* **receipt**. **Bounded** shakedown only; **no** long GPU **campaign**, **no** checkpoint **promotion**, **no** benchmark **execution**, and **no** claim **authorization** — see **`docs/starlab-v1.5.md`** (M07 non-claims) and **`docs/runtime/v15_training_smoke_short_gpu_shakedown_v1.md`**. **Intent:** `v15_training_run_receipt.json` + report; default **`fixture_ci`**; optional **`operator_declared`**; optional **`operator_local_short_gpu`** with explicit guard and isolated **synthetic** trainer (not SC2/PX2 pipelines). **Not** **V15-M08**; **`long_gpu_run_authorized` false**; merge/CI at milestone closeout.
