...
**v1.5 arc (V15) — authoritative public governance:** `docs/starlab-v1.5.md` (strategic anchor: `docs/starlab-v1.5moonshot.md`; M02 environment lock runtime: `docs/runtime/v15_long_gpu_run_environment_lock_v1.md`)
...
| Current milestone | **V15-M02** — *Long GPU Run Environment Lock* — **in progress** (not closed on `main` until merge); see **`docs/starlab-v1.5.md`**, runtime **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **Last closed (v1.5):** **`V15-M01`** on `main` ([PR #117](https://github.com/m-cahill/starlab/pull/117); merge `f618a1f…`; PR-head CI [`24914804421`](https://github.com/m-cahill/starlab/actions/runs/24914804421); merge-boundary [`24916076997`](https://github.com/m-cahill/starlab/actions/runs/24916076997)). **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** perform a **long wall-clock GPU training session** as a program deliverable. Default **CI** remains **CPU** fixture self-play / bootstrap paths. **`PX2-M04`** / **`PX2-M05`** — **planned**; **`PX1-M05`** optional; **v2** not opened. |
...
| Next planned follow-on | **`V15-M03`** (checkpoint lineage) follows **`V15-M02`** (environment lock — **in progress**). **v1.5** authority: **`docs/starlab-v1.5.md`**. **`PX2-M04`** / **`PX2-M05`** — **planned only** on the **PX2** namespace. **`PX1-M05`** optional. **`docs/starlab-vision.md`** **v2** directions remain **outside** the closed **M00–M61** arc until **explicit** recharter — **v2** is **deferred** / **not** opened. |
...
| **v1.5 (V15)** | **`V15-M00`** and **`V15-M01`** **closed** on `main` (PR #116, PR #117) — register surfaces + `starlab.v15.training_asset_registers.v1`; **not** long GPU run; **not** claim-critical asset approval. | Full GPU training session + **strong-agent** benchmark + replay-native **XAI** + checkpoint/data/**weight** provenance + **human-panel** or bounded “beats most humans” eval — **chartered in v1.5**, not in **PX2** | **V15-M02**+ |
...
**Ledger truths (PX2 phase, **closeout**; program **current** v1.5 = **`V15-M02`** (environment lock — **in progress**); **last closed** v1.5 = **`V15-M01`** / **`V15-M00`** — authority **`docs/starlab-v1.5.md`**):**
...
**Current (v1.5):** **`V15-M02`** — *Long GPU Run Environment Lock* — **in progress** (not closed on `main` until merge). **Authority:** **`docs/starlab-v1.5.md`**; runtime **`docs/runtime/v15_long_gpu_run_environment_lock_v1.md`**. **M02** defines the environment-lock contract (`starlab.v15.long_gpu_environment_lock.v1`) and **does not** execute GPU training.

### V15-M02 — Long GPU Run Environment Lock — **in progress**

**Intent:** Deterministic **environment-lock** JSON + report via `python -m starlab.v15.emit_v15_long_gpu_environment_lock --output-dir <path>`; default **`fixture_ci`**; optional **`operator_local`** with **`--probe-json`**. **Not** GPU shakedown; **not** long training; **not** claim that CI fixture output equals a real operator RTX 5090 + SC2 environment.

**Last closed (v1.5):** **`V15-M01`** — *Training-Scale Provenance and Asset Registers* — **closed** on `main` ([PR #117](https://github.com/m-cahill/starlab/pull/117); merge `f618a1f…`; **authoritative PR-head CI** [`24914804421`](https://github.com/m-cahill/starlab/actions/runs/24914804421); **merge-boundary `main` CI** [`24916076997`](https://github.com/m-cahill/starlab/actions/runs/24916076997) — **success**). **Prior:** **`V15-M00`** — **closed** ([PR #116](https://github.com/m-cahill/starlab/pull/116); merge `1391518e…`; PR-head [`24911939851`](https://github.com/m-cahill/starlab/actions/runs/24911939851); merge-boundary [`24913025515`](https://github.com/m-cahill/starlab/actions/runs/24913025515)). **Authoritative public detail:** **`docs/starlab-v1.5.md`**. **PX2** (through **`PX2-M03`**) is **closed** / **transition-complete**; **PX2** did **not** run a **long wall-clock GPU** training **campaign** as a program deliverable. **PX2-M03** readiness (historical): **`docs/runtime/px2_industrial_self_play_campaign_readiness_v1.md`**; runtime **`docs/runtime/px2_industrial_self_play_campaign_v1.md`** (slices + §8q). **Last closed PX2 milestone:** **`PX2-M03`** — prior **`PX2-M02`** ([PR #96](https://github.com/m-cahill/starlab/pull/96)). **`PX2-M04`** / **`PX2-M05`** — **planned only**. **`PX1-M05`** **optional**; **v2** **not** opened.
