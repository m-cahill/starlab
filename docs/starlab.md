...
| ----- | ------------------------ |
| Program arc | **62 milestones (M00–M61)** — see §7 (**M55**–**M58** **closed** on `main`; **M59** **merge pending** on PR; **M60**–**M61** **stub/planned** except as closed later). |
| Last closed milestone | **M58** — Live SC2 in CI hardening & cost guardrails v1 (merged to `main`; see §11 / §18). |
| Current milestone | **M59** — Ladder/public evaluation protocol & evidence surface v1 — **merge pending** (GitHub PR; **not** closed on `main` until merge). **Not** live-SC2 guardrails substitute; **not** benchmark integrity; **not** replay↔execution equivalence (**M52**–**M54** **closed**). **M58** **closed** — bounded **M57** hardening + cost guardrails — **not** global live-SC2-in-CI proof. |
...
| Still **not** proved today (present tense) | **Benchmark integrity**, **replay↔execution equivalence** (global / gameplay-semantic), **live SC2 in CI as default merge-gate / global proof**, **ladder / public performance**, **Phase VI integrated test campaign** completion (post-M45 operator work), **statistical significance of ranking** (M42 does **not** claim this), **operating manual v1** — see §11 *explicit non-claims*. These gaps are **planned within the remaining v1 arc** (**M59**–**M61**), not silently deferred to **v2** — see *Remaining v1 proof-track map* below. |
...
#### Phase VII artifact-contract index (M52–M59)
...
Planned program arc (**62 milestones**, **M00**–**M61** — **M52**–**M58** **closed** on `main` (through benchmark-integrity **charter** (**M55**) + bounded **evidence/gates** (**M56**) + narrow live-SC2-in-CI **charter + controlled runner** (**M57**) + **M58** guardrails/hardening milestones); tag **`v0.0.58-m58`** on **M58** merge commit; **M59** **in PR** / **merge pending** (not yet merged to `main`); **M60**–**M61** stub/planned; **M51** **closed** on `main`):
...
| M59 | Ladder/Public Evaluation Protocol & Evidence Surface v1 | VII | In PR | — | — |
...
| M59 | Ladder/public evaluation protocol & evidence surface v1 — **merge pending** (PR; **not** closed on `main` until merge) |
