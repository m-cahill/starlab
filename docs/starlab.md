...
| `PV1-M04` | Post-Campaign Analysis / Comparative Readout | **open** on `main` (implementation PR — **record at merge**) | Deterministic **`pv1_post_campaign_readout.json`** / report + runtime **`docs/runtime/pv1_post_campaign_readout_v1.md`** — **readout / aggregation only** — **not** new execution; **not** threshold reinterpretation |
...
### 2026-04-16 — **PV1-M04** Post-Campaign Analysis / Comparative Readout — **opened** on `main` (implementation PR — **record number + CI run IDs at merge**)

- **Delivered:** runtime **`docs/runtime/pv1_post_campaign_readout_v1.md`**; deterministic **`pv1_post_campaign_readout.json`** / **`pv1_post_campaign_readout_report.json`** via **`python -m starlab.training.emit_pv1_post_campaign_readout`** (**aggregation only**); synthetic **`tests/fixtures/pv1_m04/`** + **`tests/test_pv1_post_campaign_readout.py`** — **not** operator-local SC2 execution; **not** **`threshold-met`** fabrication; **not** reinterpretation of **`threshold-not-met`** on **`full_run_duration_target`**.
- **Post-merge ledger:** §1 quick scan (including **PV1 campaign outcome** line) + §7 **Post-v1 (PV1)** + canonical operator-artifacts subtable + §11 — **PV1-M04** **open**; **PV1-M03** remains **closed**; **`current milestone`** = **PV1-M04** until closeout.
