...
| Current milestone | **M41** — Replay-Imitation Training Pipeline v1 (**stub**; Phase VI; see §11). |
...
| Still **not** proved (default) | **Benchmark integrity**, **replay↔execution equivalence**, **live SC2 in CI**, **M41+** training pipeline / comparison / hierarchical / live-play / self-play **product** results, **operating manual v1** — see §11 *explicit non-claims*. |
...
| M41 | Replay-Imitation Training Pipeline v1 | VI | Planned | v0.0.41-m41 | — |
...
| M41 | Replay-imitation training pipeline v1 |
...
| **M41** | **Replay-imitation training pipeline v1** (planned). |
...
### M41 — Replay-Imitation Training Pipeline v1 — **stub** (current)

**Status:** **Planned / stub** — next Phase VI **training-track** milestone after **M40** closeout; **not** claimed until merged and closed on `main` with authoritative CI. Stub plan: `docs/company_secrets/milestones/M41/M41_plan.md`. **M42**–**M45** remain **stub-only** per §7.

**Explicit non-claims (standing until M41+ each closes):** **not** actual replay-imitation training pipeline product, **not** weights in-repo, **not** GPU training in CI, **not** live SC2 in CI, **not** benchmark integrity — see §1 status / §7 **M41**–**M45** rows.
