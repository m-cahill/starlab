...
| Observation surface contract v1 (single player-relative frame) | M17 | **On `main`** — narrow JSON Schema + report + fixture validation; upstream M16 canonical state; **not** materialization pipeline, perceptual bridge, or replay↔execution equivalence |
...
### Phase III schema / pipeline / observation glossary (M15–M18)
...
| **M17 observation surface contract** | **Proved (narrow, in-repo):** deterministic JSON Schema + report for one **player-relative** observation frame at one `gameloop`, bound to **M16 canonical state** as upstream — **contract + validation only**; **not** canonical→observation **materialization** (**M18** implements a **prototype**). **Proxy visibility** remains bounded (M12/M16 non-claims), not certified fog-of-war truth. |
| **M18 perceptual bridge prototype** | **Planned until closeout:** deterministic **prototype** materialization from one M16 `canonical_state.json` to one M17-shaped `observation_surface.json` + `observation_surface_report.json` (fixture-backed tests; optional report hash cross-check) — **not** replay parsing in `starlab/observation/`, **not** mask legality or benchmark claims. |
...
| **M18** | **Prototype perceptual bridge** — canonical state → one M17-shaped observation instance (materialization **planned** until M18 closeout evidence). |
...
**Status:** **Planned** — **current** milestone; **M17** is **closed** on `main` (see §18). **Phase III** is **in progress** (M15–M17 complete in milestone table; M18–M19 planned).
...
**Primary references:** `docs/company_secrets/milestones/M18/M18_plan.md`; `docs/runtime/perceptual_bridge_prototype_v1.md`; `docs/starlab.md` §6 / §10 (Phase III artifact row + glossary).

**Note:** **M17** proves **governed observation surface schema + report + validation** — **not** pixels / image-space transforms, **not** an executable action API. **M18** adds a **prototype** canonical-state→observation materialization path (still **not** full legality, **not** benchmark claims) — **planned** in the milestone table until closeout/merge evidence updates the ledger.
