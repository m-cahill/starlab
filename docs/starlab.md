...
| Canonical state schema v1 (single frame) | M15 | **On `main`** — narrow JSON Schema + report + fixture validation; not extraction pipeline, sequences, tensors, or observation API |
...
| Term | Meaning |
| ---- | ------- |
| **M15 canonical state schema** | The **governed JSON Schema** (`canonical_state_schema.json`) and **report** (`canonical_state_schema_report.json`) for a **single** replay-derived state **frame** at one `gameloop` — emission + validation only; **not** extraction from replay bundles. |
| **M16 structured state pipeline** | Intended milestone to **materialize** M15-shaped state from M09–M14 artifacts (including M14 bundles) — **not** part of M15 proof. |
| **M17 observation surface contract** | Intended milestone for **agent-facing observation** semantics — distinct from M15’s raw schema contract and from M16’s pipeline mechanics. |
...
**Status:** **Planned** — **current** milestone; **M15** is **closed** on `main` (see §18). **Phase III** is **in progress** (M15 complete; M16–M19 planned).

**Goal (high level):** Build the **structured state pipeline** that materializes M15 canonical state from replay-derived artifacts — **not** proved in M15; see Phase III map (§6) and Phase III glossary (§10).

**Primary references:** `docs/company_secrets/milestones/M16/M16_plan.md` (stub); `docs/starlab.md` §10 (proved vs not yet proved).

**Note:** **M15** canonical state schema artifacts (`canonical_state_schema.json`, `canonical_state_schema_report.json`) and validation are **proved on `main`** (narrow, schema + fixtures only). **Structured state extraction from bundles** is **not** proved until M16 closes.

#### Current milestone — explicit non-claims (standing)

Until a milestone explicitly closes a claim, treat the following as **not proved** for **M16** planning:
...
**M15 note:** Evidence column reflects **schema + report emission**, **jsonschema** validation over fixtures, and full governance CI **on `main`** (**green PR-head** [`24122064141`](https://github.com/m-cahill/starlab/actions/runs/24122064141) on `abc8ffc…`; **green merge-push** [`24122092800`](https://github.com/m-cahill/starlab/actions/runs/24122092800) on `b0f7132…`); **not** replay-to-state extraction (M16), observation contract (M17), perceptual bridge (M18), stronger economy or visibility claims than M11/M12, replay↔execution equivalence, benchmark integrity, or live SC2 in CI.
