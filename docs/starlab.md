...
| Last closed milestone (PX2 arc) | **PX2-M01** — Full Terran Runtime & Action Surface — **closed** on `main` ([PR #95](https://github.com/m-cahill/starlab/pull/95)). **Prior:** **PX2-M00** — Autonomous Full-Game Agent Charter & Success Criteria — **closed** on `main` (governance closeout [PR #94](https://github.com/m-cahill/starlab/pull/94)). **PX2-M00** — governance charter only — **not** Terran runtime. **PX2-M01** — delivered versioned Terran runtime/action substrate — **not** training; **not** strength proof. |
| Current milestone | **`PX2-M02`** — Neural Bootstrap from Replays — **open** on `main` (this milestone). **First learning milestone** over the **PX2-M01** Terran core v1 surface: conservative replay labeling, governed bootstrap dataset + report, first neural structured-action policy, legality-aware decode + compile eval, fixture-only CI — **replay-bootstrap only**; **not** self-play; **not** Blackwell execution; **not** ladder or autonomous strength proof. Public **`docs/runtime/px2_neural_bootstrap_from_replays_v1.md`**. **PX2-M01** — **closed** — **`docs/runtime/px2_full_terran_runtime_action_surface_v1.md`** + **`starlab.sc2.px2`**. **PX2-M00** — **closed** — **`docs/runtime/px2_autonomous_full_game_agent_charter_v1.md`**. **PX1-M04** — **closed** ([PR #93](https://github.com/m-cahill/starlab/pull/93)) — **does not** auto-open **`PX1-M05`** or **v2**. **`PX1-M05`** remains **optional / not yet opened**. **`PX2-M03`** — **planned only** (industrial self-play — **not** opened by **`PX2-M02`**). **v2** is **not** opened. |
...
| **PX2** | **PX2-M00** **closed** (charter). **PX2-M01** **closed** — Terran runtime substrate. **`PX2-M02`** **open** — replay-bootstrap dataset + first neural policy over M01 (**not** strength proof) | Industrial self-play (**PX2-M03**+); demo refresh — **later milestones** | **PX2-M00**–**PX2-M05** (roadmap) |
...
### PX2 roadmap (**PX2-M00** **closed**; **PX2-M01** **closed**; **`PX2-M02`** **open** on `main`; **`PX2-M03`**–**`PX2-M05`** **planned**)
...
| `PX2-M02` | Neural Bootstrap from Replays | **open** on `main` | **`docs/runtime/px2_neural_bootstrap_from_replays_v1.md`**; `starlab.sc2.px2.bootstrap` — replay labeler, dataset, first neural policy, eval — **replay-bootstrap only**; **not** self-play; **not** strength proof |
| `PX2-M03` | Industrial Self-Play Campaign | **planned** | Long Blackwell-class campaign with checkpoints/evals — **not** opened by **`PX2-M02`** |
...
**Roadmap disclaimer:** **`PX2-M03`**+ are **not** opened automatically by **`PX2-M02`**. **PX2-M02** was **not** opened automatically by **PX2-M01** closeout (separate governance). **PX2-M01** was **not** opened automatically by **PX2-M00** closeout (separate governance).
...
**Ledger truths (PX2 phase, `current milestone` = **`PX2-M02`**):**
...
- **`PX2-M02`** **open** on `main` — first **replay-bootstrap** supervised path over **M01** — **not** industrial self-play (**PX2-M03**); **not** strength proof.
- **`PX2-M03`** remains **planned only** — **not** opened by **`PX2-M02`** landing alone.
...
| PX2-M02 | First learned replay-bootstrapped policy over the **PX2-M01** Terran surface — bounded offline metrics only (**open** on `main`) |
| PX2-M03 | Execute the long industrial self-play campaign (Blackwell-class; checkpoints/evals) |
...
**Current:** **`PX2-M02`** — Neural Bootstrap from Replays — **open** on `main`. **First** PX2 **learning** milestone: conservative replay-derived labels into **`starlab.px2.terran_core.v1`**, governed **`px2_replay_bootstrap_dataset*.json`** artifacts, first **`BootstrapTerranPolicy`** (`torch`, local-first weights), legality-aware decode + **`compile_terran_action`** eval, trivial baselines — see **`docs/runtime/px2_neural_bootstrap_from_replays_v1.md`**. **In scope:** `starlab.sc2.px2.bootstrap`, fixture corpus under **`tests/fixtures/px2_m02/`**, **`tests/test_px2_m02_replay_bootstrap.py`**. **Out of scope:** self-play / RL campaigns (**PX2-M03**), Blackwell execution, ladder proof, demo refresh (**PX2-M05**), multi-race. **Non-claims:** **not** autonomous strength; **not** industrial self-play; **`PX2-M03`** remains **planned only**. **PX1-M04** — **closed** — **does not** open **`PX1-M05`** or **v2**. **`PX1-M05`** **optional / not yet opened**. **v2** **not** opened.

### PX2-M02 — Neural Bootstrap from Replays — **open** on `main`

**Intent:** Prove a **governed replay → TerranAction → neural policy → offline eval** vertical slice over the **closed PX2-M01** surface — **bounded** supervised claims only.

**Delivered / in repo:** public **`docs/runtime/px2_neural_bootstrap_from_replays_v1.md`**; Python **`starlab.sc2.px2.bootstrap`** (labeler, dataset contract, M18 flat feature adapter, policy, training/eval, dataset emitter CLI); conservative BOE-first labeling; deterministic replay-level split; CPU fixture end-to-end test.
