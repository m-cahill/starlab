...
| Current milestone | **`V15-M33`** — *candidate checkpoint model-load and CUDA inference probe* (**provisional roadmap** on **`docs/starlab-v1.5.md`** **`§V15-M33`** until chartered) — first milestone that **may** touch checkpoint blobs / model load / CUDA inference with explicit operator guards — **not** automatic benchmark pass, strength, promotion, XAI, human-panel, showcase, **v2**, or **T2/T3** |
