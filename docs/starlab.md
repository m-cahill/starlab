...
| Current milestone | **`V15-M27`** — *SC2 Rollout Duration and Training-Loop Integration Fix* — **opened** (branch **`v15-m27-sc2-rollout-training-loop-integration`**; **`starlab.v15.sc2_rollout_training_loop_integration.v1`**; **`python -m starlab.v15.run_v15_m27_sc2_rollout_training_loop_integration`**). **Operator-local outcome:** **`sc2_rollout_training_loop_integration_completed`** (dual-guard real **SC2**; **3** episodes; **`game_step` 8**; **`max_game_steps` 2048**; per-episode **`action_count`** **236 / 236 / 230** — nonzero actions, **not** **`_HarnessBot`** zero-action smoke); **`training_loop_binding.status`** **`training_update_executed`** (integration smoke **not** meaningful learning); optional **M49** bind **`preflight_ok` true**. Default **`sc2_rollout_fixture_only`** is **CI-only** — **not** this record. **`v15_m27_nontrivial_macro_smoke_policy_v1`**. **Last closed v1.5 on `main`:** **`V15-M26`** ([PR #162](https://github.com/m-cahill/starlab/pull/162); merge `393adb7f…`; **`t1_checkpoint_plumbing_completed_but_sc2_training_not_yet_meaningful`** — **§V15-M27** distinguishes **SC2 rollout + integration evidence** vs **meaningful gameplay training**). **PX2** **transition-complete**. |
