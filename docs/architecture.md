...
| M40 | `starlab.training` — M40 agent training program contract emission (`agent_training_program_contract.json` / report under `out/training_program/`); **closed** on `main` (charter milestone — **not** training results). M45 bootstrap also uses this package (separate CLI/output root). |
...
| M44 | `starlab.sc2` — local live-play validation harness (`local_live_play_validation_run.json` / report + replay binding under `out/live_validation_runs/`) — **closed** on `main`. |
| M45 | `starlab.training` — self-play / RL bootstrap (`self_play_rl_bootstrap_run.json` / report + `bootstrap_dataset.json` under `out/rl_bootstrap_runs/`; optional `updated_policy/rl_bootstrap_candidate_bundle.joblib`) — **closed** on `main`; reuses **`starlab.sc2`** M44 harness for rollouts. |

**Phase VI:** **M40**–**M45** — governed agent training, comparison, and local validation — see `docs/starlab.md` §6–§7. **M40**–**M45** are **closed** on `main` (training implementation in **`starlab.imitation`** for **M41**; **`starlab.training`** remains the cross-milestone M40 contract umbrella and hosts **M45** bootstrap; comparison harness in **`starlab.evaluation`** for **M42** — see `docs/runtime/learned_agent_comparison_harness_v1.md`; hierarchical training in **`starlab.hierarchy`** for **M43** — see `docs/runtime/hierarchical_training_pipeline_v1.md`; local live-play validation in **`starlab.sc2`** for **M44** — see `docs/runtime/local_live_play_validation_harness_v1.md`). **M45** bootstrap orchestration lives in **`starlab.training`** (see `docs/runtime/self_play_rl_bootstrap_v1.md`). The **Phase VI integrated test campaign** is **post-M45** operator-local work — see `docs/diligence/phase_vi_integrated_test_campaign.md`.
...
11. `python -m starlab.training.emit_self_play_rl_bootstrap_run --hierarchical-training-run-dir … --match-config … --output-dir out/rl_bootstrap_runs/<run_id>/ --runtime-mode fixture_stub_ci` — M45 bootstrap (fixture stub in CI; optional `--emit-updated-bundle` with `--dataset` + `--bundle-dir`; see `docs/runtime/self_play_rl_bootstrap_v1.md`).  
12. Optional: `make coverage`, `make audit`, `make lint`, `make typecheck`.  
13. Read **`docs/starlab.md`** for current milestone and non-claims.
