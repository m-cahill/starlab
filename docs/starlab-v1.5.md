...
| **V15-M14** | Evidence Remediation / Operator Evidence Acquisition — **opened / implementation** — remediation plan + operator acquisition runbook; **not** operator execution; **`docs/runtime/v15_evidence_remediation_operator_acquisition_v1.md`** |
...
| --- | --- | --- |
| V15-M14 | Evidence remediation plan and operator acquisition readiness | planning only; inventory + gates |
...
**M14 emitter:** `python -m starlab.v15.emit_v15_evidence_remediation_plan --output-dir <path>` writes `v15_evidence_remediation_plan.json`, `v15_evidence_remediation_plan_report.json`, and `v15_operator_evidence_acquisition_runbook.md`. Optional **`--m13-v2-decision-json <path>`** binds an M13 decision file by **canonical JSON SHA-256** (contract `starlab.v15.v2_go_no_go_decision.v1`); default fixture uses placeholder SHA. **M14** does not execute training, long GPU, benchmarks, or operator evidence collection.
