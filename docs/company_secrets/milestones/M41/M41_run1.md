...
**Workflow:** `CI`  
**Run ID (authoritative PR-head CI for current merge target):** [`24297190190`](https://github.com/m-cahill/starlab/actions/runs/24297190190)  
...
**Final PR head SHA (under review):** `4f855832813df2fa8739919fb281e9d449003872`  
**Branch on origin before this pass:** **No** — branch was created locally and pushed as new (`git ls-remote` was empty before push).

**Authoritative PR-head CI:** **`24297190190`** — **success** (all required jobs below passed).

**Superseded / non-authority (intermediate PR heads, not merge authority for final):**  
- [`24297168010`](https://github.com/m-cahill/starlab/actions/runs/24297168010) on `3155745…` (prior `M41_run1.md` correction)  
- [`24297148773`](https://github.com/m-cahill/starlab/actions/runs/24297148773) on `d2ddcc7…`  
...
*If the PR branch advances again, use `gh run list --branch m41-replay-imitation-training-pipeline-v1` and match `headSha` to `gh pr view 52 --json headRefOid`.*
...
- **None** on authoritative run `24297190190`.
...
**Verdict:** This PR-head CI run is **green** with all required jobs passing. It is appropriate to treat **`24297190190`** as **authoritative PR-head CI** for merge decision (subject to human approval). **Merge-boundary `main` CI** is **not** in scope until after merge.
