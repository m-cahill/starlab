...
*Further documentation-only pushes to `main` after this table may produce additional green CI runs; distinguish them in §23 — **not** merge-boundary events. The table above is complete through the last row; do not imply merge-boundary authority for those rows.*
